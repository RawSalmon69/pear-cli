...
# Tests for pear_delete in lib/core/file_ops.sh.
# Exercises permanent mode (default), trash mode (via PEAR_TEST_TRASH_DIR
...
setup() {
    SANDBOX="$(mktemp -d "${BATS_TEST_DIRNAME}/tmp-pear-delete.XXXXXX")"
    export SANDBOX
    export PEAR_DELETE_LOG="$SANDBOX/deletions.log"
    export PEAR_TEST_TRASH_DIR="$SANDBOX/Trash"
    export PEAR_TEST_NO_AUTH=1
    unset PEAR_DELETE_MODE
    unset PEAR_DRY_RUN
}
...
export PEAR_TEST_TRASH_DIR="$PEAR_TEST_TRASH_DIR"
export PEAR_TEST_NO_AUTH=1
source "$PROJECT_ROOT/lib/core/common.sh"
EOF
}

@test "pear_delete defaults to permanent mode and removes the target" {
...
EOF
    ...
    [[ -z "$(ls -A "$PEAR_TEST_TRASH_DIR" 2> /dev/null || true)" ]]
}

@test "pear_delete trash mode moves the target instead of rm -rf" {
...
EOF
    ...
    [[ -n "$(ls -A "$PEAR_TEST_TRASH_DIR" 2> /dev/null || true)" ]]
}

@test "pear_delete moves sudo-required paths to invoking user Trash" {
...
SH
    ...
    run bash --noprofile --norc <<EOF
$(prelude)
unset PEAR_TEST_TRASH_DIR
unset PEAR_TEST_NO_AUTH
export PEAR_DELETE_MODE=trash
export PATH="$fake_bin:\$PATH"
export HOME="$fake_home"
export PEAR_TEST_TRACE="$trace"
pear_delete "$victim" true
EOF
    ...
    local status_col
    status_col=$(awk -F'\t' 'END { print $4 }' "$PEAR_DELETE_LOG")
    [ "$status_col" = "ok" ]
}

@test "pear_delete refuses symlinked invoking user Trash for sudo-required paths" {
...
@test "pear_delete uses unique Trash name for sudo-required path conflicts" {
...
@test "pear_delete writes a tab-separated log line per call" {
...
EOF
    ...
    local fields
    fields=$(awk -F'\t' 'END { print NF }' "$PEAR_DELETE_LOG")
    [ "$fields" -eq 5 ]
...
@test "pear_delete rejects symlinks to protected system paths" {
...
@test "pear_delete dry-run does not touch the filesystem but still logs" {
...
@test "pear_delete records a forensic log entry for rejected paths" {
    run bash --noprofile --norc <<EOF
$(prelude)
pear_delete "/tmp/../etc/hosts"
EOF
    ...
    status_col=$(awk -F'\t' 'END { print $4 }' "$PEAR_DELETE_LOG")
    size_col=$(awk -F'\t' 'END { print $3 }' "$PEAR_DELETE_LOG")
    [ "$status_col" = "rejected" ]
    [ "$size_col" = "0" ]
}

@test "pear_delete is a no-op on a non-existent path" {
    run bash --noprofile --norc <<EOF
$(prelude)
pear_delete "$SANDBOX/does-not-exist"
EOF
...
@test "pear_delete rejects unknown delete mode without touching target" {
    local victim="$SANDBOX/invalid_mode_target"
    : > "$victim"

    run bash --noprofile --norc <<EOF
$(prelude)
export PEAR_DELETE_MODE=surprise
pear_delete "$victim"
EOF
    ...
    mode_col=$(awk -F'\t' 'END { print $2 }' "$PEAR_DELETE_LOG")
    status_col=$(awk -F'\t' 'END { print $4 }' "$PEAR_DELETE_LOG")
    [ "$mode_col" = "surprise" ]
...
@test "pear_delete warns once for repeated invalid delete mode" {
...
EOF
    ...
    [[ -e "$first" ]]
    [[ -e "$second" ]]
    [[ "$(grep -c 'invalid PEAR_DELETE_MODE' <<< "$output")" -eq 1 ]]
}

@test "pear_delete trash failure leaves target in place" {
    local victim="$SANDBOX/fallback_target"
    : > "$victim"

    # Pointing PEAR_TEST_TRASH_DIR at a non-writable parent forces the stub
...
$(prelude)
export PEAR_DELETE_MODE=trash
export PEAR_TEST_TRASH_DIR="$blocked"
pear_delete "$victim"
EOF
...
@test "pear_delete warns once for repeated Trash failures" {
...
@test "pear_delete records 'unknown' (not 0) when size measurement fails" {
...
@test "pear_delete warns once per session when audit log is unwritable" {
    ...
    run bash --noprofile --norc <<EOF
set -euo pipefail
export PEAR_DELETE_LOG="$broken_log_dir/deletions.log"
export PEAR_TEST_TRASH_DIR="$PEAR_TEST_TRASH_DIR"
export PEAR_TEST_NO_AUTH=1
source "$PROJECT_ROOT/lib/core/common.sh"
pear_delete "$victim"
...
pear_delete "$SANDBOX/second_victim"
EOF
