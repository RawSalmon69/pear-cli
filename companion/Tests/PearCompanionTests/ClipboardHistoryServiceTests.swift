...
final class ClipboardHistoryServiceTests: XCTestCase {
    // MARK: - Concealed-content recording policy

    func testMayRecordRejectsConcealedTransientAndAutoGenerated() {
        // Passwords from password managers mark the pasteboard concealed;
        // recording them would land secrets in plaintext UserDefaults.
        XCTAssertFalse(ClipboardHistoryService.mayRecord(
            types: ["public.utf8-plain-text", "org.nspasteboard.ConcealedType"]))
        XCTAssertFalse(ClipboardHistoryService.mayRecord(
            types: ["org.nspasteboard.TransientType"]))
        XCTAssertFalse(ClipboardHistoryService.mayRecord(
            types: ["org.nspasteboard.AutoGeneratedType", "public.png"]))
    }

    func testMayRecordAllowsOrdinaryContent() {
        XCTAssertTrue(ClipboardHistoryService.mayRecord(types: ["public.utf8-plain-text"]))
        XCTAssertTrue(ClipboardHistoryService.mayRecord(types: []))
    }
