#!/bin/bash
# LaunchServices cleanup helpers for `pe clean`.
...
collect_stale_launch_services_app_paths() {
    ...
    run_with_timeout "$PEAR_TIMEOUT_PKG_LIST_SEC" "$lsregister" -dump 2> /dev/null |
        launch_services_emit_missing_record_paths |
        LC_ALL=C sort -u
}
...
clean_stale_launch_services_registrations() {
    ...
    local max_items="${PEAR_LAUNCH_SERVICES_STALE_LIMIT:-50}"
    ...
    if [[ "${DRY_RUN:-false}" == "true" || "${PEAR_DRY_RUN:-0}" == "1" ]]; then
    ...
    for app_path in "${stale_apps[@]}"; do
        ...
        if run_with_timeout "$PEAR_TIMEOUT_SHORT_QUERY_SEC" "$lsregister" -u "$app_path" > /dev/null 2>&1; then
