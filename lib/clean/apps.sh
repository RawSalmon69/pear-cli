...
readonly ORPHAN_AGE_THRESHOLD=${ORPHAN_AGE_THRESHOLD:-${PEAR_ORPHAN_AGE_DAYS:-30}}
readonly CLAUDE_VM_ORPHAN_AGE_THRESHOLD=${PEAR_CLAUDE_VM_ORPHAN_AGE_DAYS:-7}
...
clean_ds_store_tree() {
    ...
    while IFS= read -r -d '' ds_file; do
        ...
        if [[ $file_count -ge $PEAR_MAX_DS_STORE_FILES ]]; then
            break
        fi
...
scan_installed_apps() {
    ...
    local cache_file="$HOME/.cache/pear/installed_apps_cache"
    ...
    (
        ...
        if [[ "${PEAR_TEST_MODE:-0}" != "1" && "${PEAR_TEST_NO_AUTH:-0}" != "1" ]]; then
            local running_apps=$(run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" osascript -e 'tell application "System Events" to get bundle identifier of every application process' 2> /dev/null || echo "")
        ...
        if command -v lsappinfo > /dev/null 2>&1; then
            run_with_timeout "$PEAR_TIMEOUT_SHORT_QUERY_SEC" lsappinfo list 2> /dev/null | grep -o '"CFBundleIdentifier"="[^"]*"' | cut -d'"' -f4 >> "$scan_tmp_dir/running.txt" 2> /dev/null || true
        fi
    ) &
    pids+=($!)
    (
        run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" find ~/Library/LaunchAgents /Library/LaunchAgents \
...
_PEAR_MDFIND_FOUND=""
_PEAR_MDFIND_NOTFOUND=""

_mdfind_cache_check() {
    local bundle_id="$1"
    local _nl=$'\n'
    case "${_nl}${_PEAR_MDFIND_FOUND}${_nl}" in
        *"${_nl}${bundle_id}${_nl}"*) return 0 ;;
    esac
    case "${_nl}${_PEAR_MDFIND_NOTFOUND}${_nl}" in
...
_mdfind_cache_store() {
    ...
    local found="$2"
    if [[ "$found" == "true" ]]; then
        _PEAR_MDFIND_FOUND="${_PEAR_MDFIND_FOUND:+${_PEAR_MDFIND_FOUND}
}${bundle_id}"
    else
        _PEAR_MDFIND_NOTFOUND="${_PEAR_MDFIND_NOTFOUND:+${_PEAR_MDFIND_NOTFOUND}
}${bundle_id}"
    fi
}
...
is_bundle_orphaned() {
    ...
    if pear_is_reverse_dns_bundle_id "$bundle_id"; then
        ...
        if [[ $_cache_rc -eq 0 ]]; then
            ...
            app_exists=$(run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" mdfind "kMDItemCFBundleIdentifier == '$bundle_id'" 2> /dev/null) || _mdfind_rc=$?
...
is_claude_vm_bundle_orphaned() {
    ...
    if [[ $_cache_rc -eq 0 ]]; then
        ...
        local app_exists _mdfind_rc=0
        app_exists=$(run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" mdfind "kMDItemCFBundleIdentifier == '$claude_bundle_id'" 2> /dev/null) || _mdfind_rc=$?
        if [[ $_mdfind_rc -ne 0 ]]; then
            return 1
        elif [[ -n "$app_exists" ]]; then
        ...
        if [[ ${#file_patterns[@]} -gt 0 ]]; then
            ...
            for item_path in "${file_patterns[@]}"; do
                ...
                for match in "${matches[@]}"; do
                    ...
                    if [[ $iteration_count -gt $PEAR_MAX_ORPHAN_ITERATIONS ]]; then
                        break
                    fi
...
clean_orphaned_system_services() {
    ...
    _system_service_app_exists() {
        ...
        if pear_is_reverse_dns_bundle_id "$bundle_id"; then
            ...
            if [[ $_cache_rc -eq 0 ]]; then
                ...
                app_found=$(run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" mdfind "kMDItemCFBundleIdentifier == '$bundle_id'" 2> /dev/null) || _mdfind_rc=$?
                if [[ $_mdfind_rc -ne 0 ]]; then
                    return 0
    ...
    if [[ $orphaned_count -gt 0 ]]; then
        ...
        for orphan_file in "${orphaned_files[@]}"; do
            ...
            # PEAR_UNINSTALL_MODE is scoped to the call and never leaks to later
            # cleanup sections; SYSTEM_CRITICAL_BUNDLES stay protected. See #1082.
            if PEAR_UNINSTALL_MODE=1 should_protect_path "$orphan_file"; then
            ...
            if [[ "$DRY_RUN" == "true" ]]; then
                ...
                elif [[ $remove_rc -eq $PEAR_ERR_PROTECTED_PATH ]]; then
                    debug_log "Skipping protected orphaned service: $orphan_file"
                    skipped_protected_count=$((skipped_protected_count + 1))
                else
...
# Policy: pe clean does NOT touch user LaunchAgents (~/Library/LaunchAgents),
        ...
        for container_dir in "$containers_dir"/$bundle_glob; do
            ...
            if [[ "$DRY_RUN" != "true" ]]; then
                ...
                if _remove_verified_container_stub "$container_dir" "$metadata_plist" > /dev/null 2>&1; then
                    removed_count=$((removed_count + 1))
                    log_operation "${PEAR_CURRENT_COMMAND:-clean}" "REMOVED" "$container_dir" "stub-container"
                else
                    debug_log "Failed to remove stub container: $container_dir"
                    failed_count=$((failed_count + 1))
                    log_operation "${PEAR_CURRENT_COMMAND:-clean}" "FAILED" "$container_dir" "stub-container"
                fi
            else
                removed_count=$((removed_count + 1))
                log_operation "${PEAR_CURRENT_COMMAND:-clean}" "SKIPPED" "$container_dir" "dry-run stub-container"
            fi
        done
    done
