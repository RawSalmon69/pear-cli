...
select_apps_for_uninstall() {
    ...
    # - PEAR_MENU_META_EPOCHS: numeric last_used_epoch per item
    # - PEAR_MENU_META_SIZEKB: numeric size in KB per item
    # The menu will gracefully fallback if these are unset or malformed.
    if [[ $has_epoch_metadata == true ]]; then
        export PEAR_MENU_META_EPOCHS="$epochs_csv"
    else
        unset PEAR_MENU_META_EPOCHS
    fi
    if [[ $has_size_metadata == true ]]; then
        export PEAR_MENU_META_SIZEKB="$sizekb_csv"
    else
        unset PEAR_MENU_META_SIZEKB
    fi
    export PEAR_MENU_FILTER_NAMES="$names_newline"
    export PEAR_MENU_IGNORE_INITIAL_ENTER=1

    # Use paginated menu - result will be stored in PEAR_SELECTION_RESULT
    ...
    unset PEAR_MENU_META_EPOCHS PEAR_MENU_META_SIZEKB PEAR_MENU_FILTER_NAMES PEAR_MENU_IGNORE_INITIAL_ENTER
    # leave PEAR_MENU_SORT_DEFAULT untouched if user set it globally

    if [[ $exit_code -ne 0 ]]; then
        return 1
    fi

    if [[ -z "$PEAR_SELECTION_RESULT" ]]; then
    ...
    IFS=',' read -r -a indices_array <<< "$PEAR_SELECTION_RESULT"
