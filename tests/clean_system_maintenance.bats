...
date +%s > "$HOME/.cache/pear/brew_last_cleanup"
...
mkdir -p "$HOME/.cache/pear"
rm -f "$HOME/.cache/pear/brew_last_cleanup"
...
mkdir -p "$HOME/.cache/pear" "$HOME/Library/Caches/Homebrew"
rm -f "$HOME/.cache/pear/brew_last_cleanup"
...
@test "run_with_timeout succeeds without GNU timeout" {
    run bash --noprofile --norc -c '
        set -euo pipefail
        PATH="/usr/bin:/bin"
        unset PE_TIMEOUT_INITIALIZED PE_TIMEOUT_BIN
        source "'"$PROJECT_ROOT"'/lib/core/common.sh"
...
@test "get_path_size_kb returns zero for missing directory" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PE_DEBUG=0 bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
