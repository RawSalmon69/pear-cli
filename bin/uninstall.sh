...
readonly PEAR_UNINSTALL_USER_LC_ALL="${LC_ALL:-}"
readonly PEAR_UNINSTALL_USER_LANG="${LANG:-}"
...
readonly PEAR_UNINSTALL_META_CACHE_DIR="$HOME/.cache/pear"
readonly PEAR_UNINSTALL_META_CACHE_FILE="$PEAR_UNINSTALL_META_CACHE_DIR/uninstall_app_metadata_v1"
readonly PEAR_UNINSTALL_META_CACHE_LOCK="${PEAR_UNINSTALL_META_CACHE_FILE}.lock"
readonly PEAR_UNINSTALL_META_REFRESH_TTL=604800 # 7 days
readonly PEAR_UNINSTALL_EPOCH_FLOOR=978307200
...
uninstall_resolve_display_name() {
    ...
    if [[ -f "$app_path/Contents/Info.plist" ]]; then
        ...
        if [[ -n "$PEAR_UNINSTALL_USER_LC_ALL" ]]; then
            md_display_name=$(run_with_timeout "$PEAR_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC" env LC_ALL="$PEAR_UNINSTALL_USER_LC_ALL" LANG="$PEAR_UNINSTALL_USER_LANG" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        elif [[ -n "$PEAR_UNINSTALL_USER_LANG" ]]; then
            md_display_name=$(run_with_timeout "$PEAR_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC" env LANG="$PEAR_UNINSTALL_USER_LANG" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        else
            md_display_name=$(run_with_timeout "$PEAR_UNINSTALL_INLINE_MDLS_DISPLAY_TIMEOUT_SEC" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        fi
    ...
    (
        _refresh_debug() {
            if [[ "${PE_DEBUG:-}" == "1" ]]; then
                ...
                echo "[$ts] DEBUG: [metadata-refresh] $*" >> "${HOME}/.config/pear/pear_debug_session.log" 2> /dev/null || true
            fi
        }

        ensure_user_dir "$PEAR_UNINSTALL_META_CACHE_DIR"
        ensure_user_file "$PEAR_UNINSTALL_META_CACHE_FILE"
        if [[ ! -r "$PEAR_UNINSTALL_META_CACHE_FILE" ]]; then
            if ! : > "$PEAR_UNINSTALL_META_CACHE_FILE" 2> /dev/null; then
                _refresh_debug "Cannot create cache file, aborting"
                exit 0
            fi
        fi
        if [[ ! -w "$PEAR_UNINSTALL_META_CACHE_FILE" ]]; then
        ...
        while IFS='|' read -r app_path app_mtime bundle_id display_name; do
            ...
            (
                ...
                if [[ ! "$last_used_epoch" =~ ^[0-9]+$ || $last_used_epoch -le 0 || $last_used_epoch -lt $PEAR_UNINSTALL_EPOCH_FLOOR ]]; then
                    last_used_epoch=0
                fi
        ...
        if ! uninstall_acquire_metadata_lock "$PEAR_UNINSTALL_META_CACHE_LOCK"; then
        ...
        refresh_merged_file=$(mktemp 2> /dev/null) || {
            ...
            uninstall_release_metadata_lock "$PEAR_UNINSTALL_META_CACHE_LOCK"
            rm -f "$updates_file"
            exit 0
        }
        ...
        ' "$updates_file" "$PEAR_UNINSTALL_META_CACHE_FILE" > "$refresh_merged_file"

        uninstall_persist_cache_file "$refresh_merged_file" "$PEAR_UNINSTALL_META_CACHE_FILE"
...
#         ${temp_file}.sorted, PEAR_UNINSTALL_META_CACHE_FILE
...
_scan_finalize_index() {
    ...
    awk -F'|' \
        -v now="$current_epoch" \
        -v floor="$PEAR_UNINSTALL_EPOCH_FLOOR" \
        -v ttl="$PEAR_UNINSTALL_META_REFRESH_TTL" \
    ...
    if [[ -s "$cache_snapshot_file" ]]; then
        if uninstall_acquire_metadata_lock "$PEAR_UNINSTALL_META_CACHE_LOCK"; then
            uninstall_persist_cache_file "$cache_snapshot_file" "$PEAR_UNINSTALL_META_CACHE_FILE"
            uninstall_release_metadata_lock "$PEAR_UNINSTALL_META_CACHE_LOCK"
        fi
    fi
...
scan_applications() {
    ...
    ensure_user_dir "$PEAR_UNINSTALL_META_CACHE_DIR"
    ensure_user_file "$PEAR_UNINSTALL_META_CACHE_FILE"
    local cache_source="$PEAR_UNINSTALL_META_CACHE_FILE"
    ...
    start_scan_spinner() {
        ...
        [[ -t 2 || "${PEAR_TEST_FORCE_SCAN_SPINNER:-0}" == "1" ]] || return 0
        (
            # shellcheck disable=SC2329  # Function invoked indirectly via trap
...
start_uninstall_interactive_screen() {
    if [[ -t 1 && -t 2 && "${PEAR_ALT_SCREEN_ACTIVE:-}" != "1" ]]; then
        enter_alt_screen
...
stop_uninstall_interactive_screen() {
    if [[ "${PEAR_ALT_SCREEN_ACTIVE:-}" == "1" ]]; then
        leave_alt_screen
    fi
...
# the exact name `pe uninstall` accepts, and human-readable size. Reuses the
...
uninstall_list_apps() {
    ...
    # Auto-switch to JSON when stdout is piped, matching `pe status`.
    local format="text"
    if [[ ! -t 1 ]]; then
        format="json"
    fi
    ...
    printf '\n%d application(s)  |  Remove with: pe uninstall <UNINSTALL NAME>\n\n' "$total"
    return 0
}

main() {
    # Set current command for operation logging
    export PEAR_CURRENT_COMMAND="uninstall"
    ...
    export PEAR_DELETE_MODE="${PEAR_DELETE_MODE:-trash}"
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            "--dry-run" | "-n")
                ...
                ;;
            "--permanent")
                export PEAR_DELETE_MODE="permanent"
                ;;
            "--list")
                list_mode=1
                ;;
            "--whitelist")
                ...
                echo "Whitelist management is currently supported by: pe clean --whitelist / pe optimize --whitelist"
                echo "Use 'pe uninstall --help' for supported options."
                exit 1
                ;;
            -*)
                echo "Unknown uninstall option: $arg"
    ...
    unset PEAR_INLINE_LOADING PEAR_MANAGED_ALT_SCREEN PEAR_ALT_SCREEN_ACTIVE
    while true; do
        unset PEAR_INLINE_LOADING
