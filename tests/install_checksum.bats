...
stubdir="$(mktemp -d "${TMPDIR:-/tmp}/pear-gh-stub.XXXXXX")"
cat > "$stubdir/gh" <<'STUB'
#!/bin/bash
case "$1 $2" in
...
STUB
chmod +x "$stubdir/gh"
target="$(mktemp "${TMPDIR:-/tmp}/pear-att-file.XXXXXX")"
...
file="$(mktemp "${TMPDIR:-/tmp}/pear-asset.XXXXXX")"
...
# cannot verify (status 2) + PEAR_REQUIRE_ATTESTATION=1 -> fatal
verify_release_attestation() { return 2; }
out="$(PEAR_REQUIRE_ATTESTATION=1 verify_release_asset_checksum V1.0.0 "$asset" "$file")" && rc=0 || rc=$?
[ "$rc" -eq 1 ] || { echo "WRONG: require-gate rc=$rc want 1"; exit 1; }
[[ "$out" == *"ERROR:PEAR_REQUIRE_ATTESTATION=1 set but gh"* ]] || { echo "WRONG: require-gate error missing: $out"; exit 1; }
