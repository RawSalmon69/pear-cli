...
gpu_cache_dir_is_stale() {
    local cache_dir="$1"
    local age_days="${2:-${PEAR_GPU_CACHE_AGE_DAYS:-1}}"
...
clean_deep_system() {
    ...
    if sudo -n test -d "/Library/Caches" 2> /dev/null; then
        while IFS= read -r -d '' file; do
            ...
            \( -name "*.cache" -mtime "+$PEAR_TEMP_FILE_AGE_DAYS" \) -o \
            \( -name "*.tmp" -mtime "+$PEAR_TEMP_FILE_AGE_DAYS" \) -o \
            \( -name "*.log" -mtime "+$PEAR_LOG_AGE_DAYS" \) \
    ...
    for tmp_dir in "${sys_temp_dirs[@]}"; do
        if sudo -n find "$tmp_dir" -maxdepth 1 -type f -mtime "+${PEAR_TEMP_FILE_AGE_DAYS}" -print -quit 2> /dev/null | grep -q .; then
            if safe_sudo_find_delete "$tmp_dir" "*" "${PEAR_TEMP_FILE_AGE_DAYS}" "f"; then
                tmp_cleaned=1
            fi
        fi
    done
    ...
    if sudo -n find "/Library/Logs/DiagnosticReports" -maxdepth 1 -type f -mtime "+$PEAR_CRASH_REPORT_AGE_DAYS" -print -quit 2> /dev/null | grep -q .; then
        safe_sudo_find_delete "/Library/Logs/DiagnosticReports" "*" "$PEAR_CRASH_REPORT_AGE_DAYS" "f" || true
    fi
    stop_section_spinner
    log_success "System crash reports"
    start_section_spinner "Cleaning system logs..."
    if sudo -n find "/private/var/log" -maxdepth 3 -type f \( -name "*.log" -o -name "*.gz" -o -name "*.asl" \) -mtime "+$PEAR_LOG_AGE_DAYS" -print -quit 2> /dev/null | grep -q .; then
        safe_sudo_find_delete "/private/var/log" "*.log" "$PEAR_LOG_AGE_DAYS" "f" || true
        safe_sudo_find_delete "/private/var/log" "*.gz" "$PEAR_LOG_AGE_DAYS" "f" || true
        safe_sudo_find_delete "/private/var/log" "*.asl" "$PEAR_LOG_AGE_DAYS" "f" || true
    fi
    stop_section_spinner
    ...
    for third_party_log_dir in "${third_party_log_dirs[@]}"; do
        if sudo -n test -d "$third_party_log_dir" 2> /dev/null; then
            if sudo -n find "$third_party_log_dir" -maxdepth 5 -type f -mtime "+$PEAR_LOG_AGE_DAYS" -print -quit 2> /dev/null | grep -q .; then
                if safe_sudo_find_delete "$third_party_log_dir" "*" "$PEAR_LOG_AGE_DAYS" "f"; then
                    third_party_logs_cleaned=1
                fi
            fi
        fi
    done
    if sudo -n find "/Library/Logs" -maxdepth 1 -type f -name "adobegc.log" -mtime "+$PEAR_LOG_AGE_DAYS" -print -quit 2> /dev/null | grep -q .; then
    ...
    done < <(run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" command find /private/var/folders -maxdepth 5 -type d -name "*.code_sign_clone" -path "*/X/*" -print0 2> /dev/null || true)
    stop_section_spinner
    ...
    while IFS= read -r -d '' gpu_cache_dir; do
        ...
        gpu_cache_dir_is_stale "$gpu_cache_dir" "$PEAR_GPU_CACHE_AGE_DAYS" || continue
    ...
    safe_sudo_find_delete "$diag_base" "*" "$PEAR_LOG_AGE_DAYS" "f" || true
    safe_sudo_find_delete "$diag_base" "*.tracev3" "30" "f" || true
    safe_sudo_find_delete "/private/var/db/DiagnosticPipeline" "*" "$PEAR_LOG_AGE_DAYS" "f" || true
    stop_section_spinner
    log_success "System diagnostic logs"

    start_section_spinner "Cleaning power logs..."
    safe_sudo_find_delete "/private/var/db/powerlog" "*" "$PEAR_LOG_AGE_DAYS" "f" || true
    stop_section_spinner
...
clean_time_machine_failed_backups() {
    ...
    local tm_info
    tm_info=$(run_with_timeout "$PEAR_TIMEOUT_QUICK_DETECT_SEC" tmutil destinationinfo 2>&1 || echo "failed")
    ...
    for volume in "${backup_volumes[@]}"; do
        ...
        if [[ -d "$backupdb_dir" ]]; then
            while IFS= read -r inprogress_file; do
                ...
                if [[ $hours_old -lt $PEAR_TM_BACKUP_SAFE_HOURS ]]; then
                    continue
                fi
                local size_kb
