...
readonly PEAR_TM_THIN_TIMEOUT=180
readonly PEAR_TM_THIN_VALUE=9999999999
readonly PEAR_SQLITE_MAX_SIZE=104857600 # 100MB
...
# Test-mode env vars hard-deny so ad-hoc task calls under PEAR_TEST_NO_AUTH=1
...
optimize_sudo_available() {
    ...
    [[ "${PEAR_OPTIMIZE_SUDO_AVAILABLE:-true}" == "true" ]]
}
...
has_active_vpn_interface() {
    case "${PEAR_ASSUME_VPN_ACTIVE:-}" in
        1 | true | TRUE | yes | YES)
            return 0
            ;;
        0 | false | FALSE | no | NO)
...
opt_saved_state_cleanup() {
    if [[ "${PE_DEBUG:-}" == "1" ]]; then
        ...
        debug_operation_detail "Method" "Find and remove .savedState folders older than $PEAR_SAVED_STATE_AGE_DAYS days"
    ...
    if [[ -d "$state_dir" ]]; then
        ...
        done < <(command find "$state_dir" -type d -name "*.savedState" -mtime "+$PEAR_SAVED_STATE_AGE_DAYS" -print0 2> /dev/null)
    fi
...
opt_fix_broken_configs() {
    ...
    if [[ -t 1 ]]; then
        PEAR_SPINNER_PREFIX="  " start_inline_spinner "Checking preferences..."
...
opt_quarantine_cleanup() {
    ...
    local row_count
    row_count=$(run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" sqlite3 "$quarantine_db" "SELECT COUNT(*) FROM LSQuarantineEvent;" 2> /dev/null || echo "0")
    ...
    if [[ "${PEAR_DRY_RUN:-0}" != "1" ]]; then
        local exit_code=0
        set +e
        run_with_timeout "$PEAR_TIMEOUT_PKG_LIST_SEC" sqlite3 "$quarantine_db" "DELETE FROM LSQuarantineEvent; VACUUM;" 2> /dev/null
        exit_code=$?
        set -e
...
opt_sqlite_vacuum() {
    ...
    local spinner_started="false"
    if [[ "${PEAR_DRY_RUN:-0}" != "1" && -t 1 ]]; then
        PEAR_SPINNER_PREFIX="  " start_inline_spinner "Optimizing databases..."
        spinner_started="true"
    fi
    ...
    for pattern in "${db_paths[@]}"; do
        while IFS= read -r db_file; do
            ...
            if [[ "$file_size" -gt "$PEAR_SQLITE_MAX_SIZE" ]]; then
                skipped=$((skipped + 1))
                continue
            fi
            ...
            local page_info=""
            page_info=$(run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" sqlite3 "$db_file" "PRAGMA page_count; PRAGMA freelist_count;" 2> /dev/null || echo "")
            local page_count=""
            ...
            if [[ "${PEAR_DRY_RUN:-0}" != "1" ]]; then
                ...
                set +e
                integrity_check=$(run_with_timeout "$PEAR_TIMEOUT_PKG_LIST_SEC" sqlite3 "$db_file" "PRAGMA integrity_check;" 2> /dev/null)
            ...
            if [[ "${PEAR_DRY_RUN:-0}" != "1" ]]; then
                set +e
                run_with_timeout "$PEAR_TIMEOUT_PKG_CLEANUP_SEC" sqlite3 "$db_file" "VACUUM;" 2> /dev/null
                exit_code=$?
                set -e
...
opt_launch_services_rebuild() {
    ...
    if [[ -t 1 ]]; then
        PEAR_SPINNER_PREFIX="  " start_inline_spinner "Repairing LaunchServices..."
    fi
...
opt_memory_pressure_relief() {
    ...
    if [[ "${PEAR_DRY_RUN:-0}" != "1" ]]; then
        ...
        local spinner_started="false"
        if [[ -t 1 ]]; then
            PEAR_SPINNER_PREFIX="  " start_inline_spinner "Releasing inactive memory..."
            spinner_started="true"
        fi
...
opt_spotlight_index_optimize() {
    local spotlight_status
    spotlight_status=$(run_with_timeout "$PEAR_TIMEOUT_SHORT_QUERY_SEC" mdutil -s / 2> /dev/null || echo "")
    ...
    if echo "$spotlight_status" | grep -qi "Indexing enabled" && ! echo "$spotlight_status" | grep -qi "Indexing and searching disabled"; then
        ...
        local slow_threshold="${PEAR_OPTIMIZE_SPOTLIGHT_SLOW_SEC:-3}"
        ...
        local spinner_started="false"
        if [[ -t 1 ]]; then
            PEAR_SPINNER_PREFIX="  " start_inline_spinner "Checking Spotlight speed..."
            spinner_started="true"
        fi
        ...
        for probe in 1 2; do
            ...
            run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" mdfind "kMDItemFSName == 'Applications'" > /dev/null 2>&1 || true
    ...
    while entry=$(/usr/libexec/PlistBuddy -c "Print :EnabledPreferenceRules:$i" "$plist" 2> /dev/null); do
        case "$entry" in
            ...
            *)
                ...
                if pear_is_reverse_dns_bundle_id "$entry" && ! bundle_has_installed_app "$entry"; then
                    removed+=("$entry")
                else
                    keep+=("$entry")
                fi
...
# Log path is configurable via PEAR_PERIODIC_LOG for testing; defaults to /var/log/daily.out.
...
opt_periodic_maintenance() {
    ...
    local daily_log="${PEAR_PERIODIC_LOG:-/var/log/daily.out}"
    local stale_days=7
    ...
    if [[ "${PEAR_DRY_RUN:-0}" != "1" ]]; then
        if [[ "${PEAR_TEST_MODE:-0}" == "1" || "${PEAR_TEST_NO_AUTH:-0}" == "1" ]] || ! optimize_sudo_available; then
...
# causing the system to freeze. Set PEAR_ENABLE_DISK_VERIFY=1 to opt in.
opt_disk_verify() {
    if [[ "${PEAR_ENABLE_DISK_VERIFY:-0}" != "1" ]]; then
        opt_msg "Disk verify skipped (set PEAR_ENABLE_DISK_VERIFY=1 to enable)"
        return 0
    fi
    ...
    if [[ -t 1 ]]; then
        PEAR_SPINNER_PREFIX="  " start_inline_spinner "Verifying disk filesystem..."
    fi
    local output
    output=$(run_with_timeout "$PEAR_TIMEOUT_DISK_VERIFY_SEC" diskutil verifyVolume / 2>&1 || true)
    if [[ -t 1 ]]; then
        stop_inline_spinner
    fi
...
_login_item_debug() {
    if [[ "${PE_DEBUG:-}" == "1" ]] && declare -f debug_log > /dev/null 2>&1; then
...
_login_item_app_exists() {
    ...
    local btm_path=""
    if [[ "${PEAR_TEST_MODE:-0}" != "1" && "${PEAR_TEST_NO_AUTH:-0}" != "1" ]] && sudo -n true 2> /dev/null; then
