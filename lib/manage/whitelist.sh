...
_PEAR_MANAGE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$_PEAR_MANAGE_DIR/../core/common.sh"
source "$_PEAR_MANAGE_DIR/../ui/menu_simple.sh"

# Config file paths
readonly WHITELIST_CONFIG_CLEAN="$HOME/.config/pear/whitelist"
readonly WHITELIST_CONFIG_OPTIMIZE="$HOME/.config/pear/whitelist_optimize"
readonly WHITELIST_CONFIG_OPTIMIZE_LEGACY="$HOME/.config/pear/whitelist_checks"
...
${GRAY}Edit: ${display_config}${NC}"
    ...
    if [[ ${#selected_cache_patterns[@]} -gt 0 ]]; then
        ...
        local IFS=','
        export PEAR_PRESELECTED_INDICES="${preselected_indices[*]}"
    else
        unset PEAR_PRESELECTED_INDICES
    fi
    ...
    if [[ -n "$PEAR_SELECTION_RESULT" ]]; then
        local -a selected_indices
        IFS=',' read -ra selected_indices <<< "$PEAR_SELECTION_RESULT"
