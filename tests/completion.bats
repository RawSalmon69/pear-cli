...
@test "completion bash generates valid bash script" {
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"_pear_completions"* ]]
	[[ "$output" == *"complete -F _pear_completions pear pe"* ]]
}
...
@test "completion bash script supports pe command" {
	run "$PROJECT_ROOT/bin/completion.sh" bash
	[ "$status" -eq 0 ]
	[[ "$output" == *"complete -F _pear_completions pear pe"* ]]
}
...
@test "completion bash can be loaded in bash" {
	run bash -c "eval \"\$(\"$PROJECT_ROOT/bin/completion.sh\" bash)\" && complete -p pear"
	[ "$status" -eq 0 ]
	[[ "$output" == *"_pear_completions"* ]]
}
...
@test "completion fish generates valid fish script" {
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"complete -f -c pear"* ]]
	[[ "$output" == *"complete -f -c pe"* ]]
}
...
@test "completion auto-install detects zsh" {
	...
	run grep -E "pear[[:space:]]+completion" "$HOME/.zshrc"
	[ "$status" -eq 0 ]
}
...
@test "completion auto-install detects already installed" {
	...
	echo 'eval "$(pear completion zsh)"' >"$HOME/.zshrc"
