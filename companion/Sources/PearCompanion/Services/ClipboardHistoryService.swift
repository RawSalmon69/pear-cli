...
final class ClipboardHistoryService {
    ...
    func copy(_ item: ClipItem) {
        ...
        // Advancing lastChangeCount is what suppresses re-recording this write;
        // the old boolean guard alongside it never got consumed (the changeCount
        // check short-circuits first) and then swallowed the NEXT app's copy.
    ...
    /// Pasteboard marker types that mean "do not record": concealed
    /// (passwords from password managers / Keychain), transient, and
    /// auto-generated content. The nspasteboard.org convention Maccy honors;
    /// recording these would drop secrets into plaintext UserDefaults.
    ...
    /// Pure policy: whether a pasteboard bearing these types may be recorded.
    static func mayRecord(types: [String]) -> Bool {
        !types.contains { excludedPasteboardTypes.contains($0) }
    }
    ...
    private func poll() {
        ...
        guard Self.mayRecord(types: (pasteboard.types ?? []).map(\.rawValue)) else { return }
