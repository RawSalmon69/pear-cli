...
if [[ -n "${PEAR_LOG_LOADED:-}" ]]; then
    return 0
fi
readonly PEAR_LOG_LOADED=1
...
readonly LOG_FILE="${HOME}/Library/Logs/pear/pear.log"
readonly DEBUG_LOG_FILE="${HOME}/Library/Logs/pear/pear_debug_session.log"
readonly OPERATIONS_LOG_FILE="${HOME}/Library/Logs/pear/operations.log"
...
rotate_log_once() {
    ...
    [[ -n "${PEAR_LOG_ROTATED:-}" ]] && return 0
    export PEAR_LOG_ROTATED=1
...
# Phase-level performance timing, gated behind PE_DEBUG=1.
...
log_system_info() {
    ...
    [[ -n "${PEAR_SYS_INFO_LOGGED:-}" ]] && return 0
    export PEAR_SYS_INFO_LOGGED=1
    ...
    {
        echo "----------------------------------------------------------------------"
        echo "Pear Debug Session, $(date '+%Y-%m-%d %H:%M:%S')"
        echo "----------------------------------------------------------------------"
        echo "User: $USER"
