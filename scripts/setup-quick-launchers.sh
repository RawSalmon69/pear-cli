#!/bin/bash
# Create Raycast script commands and Alfred keywords for Pear (clean + uninstall).
...
LAUNCHER_COMMAND_SPECS=(
    "clean|Pear Clean|Deep system cleanup with Pear|Run Pear clean"
    "uninstall|Pear Uninstall|Uninstall applications with Pear|Uninstall apps via Pear"
    "optimize|Pear Optimize|System health checks and optimization|System health and optimization"
    "analyze|Pear Analyze|Disk space analysis with Pear|Disk space analysis"
    "status|Pear Status|Live system status dashboard|Live system dashboard"
)
...
detect_mo() {
    if command -v pe > /dev/null 2>&1; then
        command -v pe
    elif command -v pear > /dev/null 2>&1; then
        command -v pear
    else
        log_error "Pear not found. Install it first via Homebrew or ./install.sh."
        exit 1
    fi
}
...
write_raycast_script() {
    ...
    local cmd_for_applescript="${pe_bin//\\/\\\\}"
...
PE_BIN="${pe_bin}"
PE_SUBCOMMAND="${subcommand}"
PE_BIN_ESCAPED="${cmd_for_applescript}"
...
detect_launcher_app() {
    if [[ -n "\${PE_LAUNCHER_APP:-}" ]]; then
        echo "\${PE_LAUNCHER_APP}"
        return
    fi
...
set targetCommand to "\${PE_BIN_ESCAPED} \${PE_SUBCOMMAND}"
...
APPLESCRIPT
        ...
        Alacritty)
            if launcher_available "Alacritty" && command -v open >/dev/null 2>&1; then
                open -na "Alacritty" --args -e /bin/zsh -lc "\"\${PE_BIN}\" \${PE_SUBCOMMAND}"
                return \$?
            fi
            ;;
        Kitty)
            if has_bin "kitty"; then
                kitty --hold /bin/zsh -lc "\"\${PE_BIN}\" \${PE_SUBCOMMAND}"
                return \$?
            elif [[ -x "/Applications/kitty.app/Contents/MacOS/kitty" ]]; then
                "/Applications/kitty.app/Contents/MacOS/kitty" --hold /bin/zsh -lc "\"\${PE_BIN}\" \${PE_SUBCOMMAND}"
                return \$?
            fi
            ;;
        WezTerm)
            if has_bin "wezterm"; then
                wezterm start -- /bin/zsh -lc "\"\${PE_BIN}\" \${PE_SUBCOMMAND}"
                return \$?
            elif [[ -x "/Applications/WezTerm.app/Contents/MacOS/wezterm" ]]; then
                "/Applications/WezTerm.app/Contents/MacOS/wezterm" start -- /bin/zsh -lc "\"\${PE_BIN}\" \${PE_SUBCOMMAND}"
                return \$?
            fi
            ;;
        Ghostty)
            if launcher_available "Ghostty" && command -v open >/dev/null 2>&1; then
                open -na "Ghostty" --args -e /bin/zsh -lc "\"\${PE_BIN}\" \${PE_SUBCOMMAND}; exec /bin/zsh -l"
                return \$?
            fi
            ;;
        Hyper)
            if launcher_available "Hyper" && command -v open >/dev/null 2>&1; then
                open -na "Hyper" --args /bin/zsh -lc "\"\${PE_BIN}\" \${PE_SUBCOMMAND}"
                return \$?
            fi
            ;;
        WindTerm)
            if launcher_available "WindTerm" && command -v open >/dev/null 2>&1; then
                open -na "WindTerm" --args /bin/zsh -lc "\"\${PE_BIN}\" \${PE_SUBCOMMAND}"
                return \$?
            fi
            ;;
        Warp)
            if launcher_available "Warp" && command -v open >/dev/null 2>&1; then
                open -na "Warp" --args /bin/zsh -lc "\"\${PE_BIN}\" \${PE_SUBCOMMAND}"
                return \$?
            fi
            ;;
    esac
...
create_raycast_commands() {
    ...
    for entry in "${LAUNCHER_COMMAND_SPECS[@]}"; do
        ...
        write_raycast_script "$dir/pear-${subcommand}.sh" "$title" "$description" "$pe_bin" "$subcommand"
    done
...
create_alfred_workflow() {
    ...
    for entry in "${LAUNCHER_COMMAND_SPECS[@]}"; do
        ...
        bundle="fun.rawsalmon69.pear.${subcommand}"
        keyword="${subcommand}"
        command="\"${pe_bin}\" ${subcommand}"
...
main() {
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "  Pear Quick Launchers"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"

    local pe_bin
    pe_bin="$(detect_mo)"
    log_step "Detected Pear binary at: ${pe_bin}"

    create_raycast_commands "$pe_bin"
    create_alfred_workflow "$pe_bin"
