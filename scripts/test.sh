...
TEST_SYSTEM_STUB_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pear-test-stubs.XXXXXX")"
TEST_GO_HELPER_DIR=""
...
printf 'pear test blocked sudo: %s\n' "$*" >&2
exit 1
EOF

cat > "$TEST_SYSTEM_STUB_DIR/osascript" << 'EOF'
#!/bin/bash
printf 'pear test blocked osascript: %s\n' "$*" >&2
exit 1
EOF

cat > "$TEST_SYSTEM_STUB_DIR/launchctl" << 'EOF'
#!/bin/bash
printf 'pear test blocked launchctl: %s\n' "$*" >&2
exit 0
EOF
...
GO_TEST_CACHE="${PEAR_GO_TEST_CACHE:-/tmp/pear-go-build-cache}"
export PEAR_GO_TEST_CACHE="$GO_TEST_CACHE"
...
prepare_go_test_helpers() {
    ...
    TEST_GO_HELPER_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pear-go-helpers.XXXXXX")"
    ...
    if GOCACHE="$GO_TEST_CACHE" go build -o "$TEST_GO_HELPER_DIR/analyze-go" ./cmd/analyze > /dev/null 2>&1 &&
        ...
        export PEAR_TEST_ANALYZE_BIN="$TEST_GO_HELPER_DIR/analyze-go"
        export PEAR_TEST_STATUS_BIN="$TEST_GO_HELPER_DIR/status-go"
    else
        rm -rf "$TEST_GO_HELPER_DIR"
        TEST_GO_HELPER_DIR=""
    fi
...
if command -v bats > /dev/null 2>&1 && [ -d "tests" ]; then
    ...
    if $bats_has_jobs && { command -v parallel > /dev/null 2>&1 || command -v rush > /dev/null 2>&1; }; then
        ...
        if [[ "${PEAR_TEST_JOBS:-}" =~ ^[0-9]+$ && "${PEAR_TEST_JOBS:-0}" -gt 0 ]]; then
            _jobs="$PEAR_TEST_JOBS"
        else
...
if bash -n pear && bash -n bin/clean.sh && bash -n bin/optimize.sh; then
...
if [[ "$(uname -s)" != "Darwin" ]]; then
    ...
    # Skip if Homebrew pear is installed (install.sh will refuse to overwrite)
    install_test_home=""
    if command -v brew > /dev/null 2>&1 && brew list pear &> /dev/null; then
        printf "${GREEN}${ICON_SUCCESS} Installation test skipped, Homebrew${NC}\n"
    else
        install_test_home="$(mktemp -d /tmp/pear-test-home.XXXXXX 2> /dev/null || true)"
        if [[ -z "$install_test_home" ]]; then
            install_test_home="/tmp/pear-test-home"
    ...
    if [[ -z "$install_test_home" ]]; then
        ...
        PE_NO_OPLOG=1 \
        ./install.sh --prefix /tmp/pear-test > /dev/null 2>&1; then
        if [[ -f "/tmp/pear-test/pear" ]]; then
    ...
    PE_NO_OPLOG=1 safe_remove "/tmp/pear-test" true || true
    if [[ -n "$install_test_home" ]]; then
        PE_NO_OPLOG=1 safe_remove "$install_test_home" true || true
    fi
fi
echo ""
