...
// Original file: Loop/Window Management/Window Manipulation/WindowTransformAnimation.swift
//
// The animation driver is Loop's, verbatim: an `NSAnimation` subclass whose
// `frameRate` is tied to the display's refresh rate, driven `.nonblocking`,
// applying a hand-rolled cubic ease-out (`1 - pow(1 - progress, 3)`) per tick
// via `apply(progress:)`. The valuable part — mid-flight re-anchoring for apps
// that clamp size (fixed aspect ratio / fixed axis detection) so the resizing
// edge stays pinned — is carried over unchanged.
//
// Adaptations for Pear:
//   • Loop's `Window` becomes `AXWindowHandle` (frame / setPosition / setSize
//     over an AXUIElement); see WindowEngine.swift.
//   • Loop's per-`CGWindowID` `activeAnimationByWindow` dedup dictionary is
//     dropped — WindowEngine holds the single in-flight animation and cancels
//     it before starting a new one, so no private `_AXUIElementGetWindow` is
//     needed to key the ring's one-window snaps.
//   • The 0.3 s duration is parameterized so the speed picker can scale it.
//   • Scribe logging removed. Loop marks the class `@MainActor`; under Swift 6
//     strict concurrency that collides with `NSAnimation`'s nonisolated
//     overridable members, so this class is left nonisolated instead — it only
//     ever runs on the main run loop (`.nonblocking`, started from the main
//     actor), and it touches no actor-isolated state, so there is no race.
//   • `frameRate` reads `NSScreen.maximumFramesPerSecond` (the native refresh
//     rate) rather than Loop's `displayMode` extension.
...
final class WindowTransformAnimation: NSAnimation {
    ...
    private let window: AXWindowHandle
    ...
    /// Pure cubic ease-out, extracted so the curve can be unit-tested.
    ...
    init(
        ...
        self.frameRate = Float(NSScreen.main?.maximumFramesPerSecond ?? 60)
    ...
    private func apply(progress: Float) {
        let value = CGFloat(Self.easeOutCubic(Double(progress)))
