...
- **Capture (v1)**: global hotkey ⌃⇧P + panel button → `screencapture -i` (native region select) → auto-copy to clipboard + auto-save to a user-chosen folder (default `~/Pictures/Pear Screenshots`) → floating preview (bottom corner, ~6 s, hover to keep) with Copy / Show in Finder / **Send 🍐** buttons. Send = encrypted image message to the other Mac. CleanShot-lite for the two of us; fixes the save-location annoyance natively.
