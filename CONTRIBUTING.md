...
Modules check the internal `PE_DEBUG` variable:
...
- Go 1.24+ is required to build the `pe status` or `pe analyze` TUI binaries locally.

## Go Components

`pe status` and `pe analyze` use Go with Bubble Tea for interactive dashboards.
