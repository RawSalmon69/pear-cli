...
@test "compact_purge_scan_path keeps the tail of long purge paths visible" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_SKIP_MAIN=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
...
grep -q "^~/" "$HOME/.config/pear/purge_paths"
EOF
...
@test "scan_purge_targets: finds direct-child artifacts in project root with find mode" {
	...
	result=$(bash -c "
        source '$PROJECT_ROOT/lib/clean/project.sh'
        PE_USE_FIND=1 scan_purge_targets '$HOME/single-project' '$scan_output'
...
@test "scan_purge_targets: includes valid CACHEDIR.TAG directories in find mode" {
	...
	result=$(bash -c "
        source '$PROJECT_ROOT/lib/clean/project.sh'
        PE_USE_FIND=1 scan_purge_targets '$HOME/www' '$scan_output'
...
@test "scan_purge_targets: trusts empty fd result without falling back to find" {
	mkdir -p "$HOME/.config/pear" "$HOME/www/empty-project"
	printf '%s\n' "$HOME/www" > "$HOME/.config/pear/purge_paths"
...
mkdir -p "$HOME/.cache/pear"
echo "0" > "$HOME/.cache/pear/purge_stats"
...
export PEAR_PURGE_INCLUDE_EMPTY=1
export PEAR_DRY_RUN=1
clean_project_artifacts </dev/null

stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/pear"
...
mkdir -p "$artifact" "$HOME/.cache/pear"
...
@test "pe purge: command exists and is executable" {
	[ -x "$PROJECT_ROOT/pear" ]
	[ -f "$PROJECT_ROOT/bin/purge.sh" ]
}

@test "pe purge: shows in help text" {
	run env HOME="$HOME" "$PROJECT_ROOT/pear" --help
	[ "$status" -eq 0 ]
	[[ "$output" == *"pear purge"* ]]
}

@test "pe purge --help includes include-empty option" {
	run env HOME="$HOME" "$PROJECT_ROOT/pear" purge --help
	[ "$status" -eq 0 ]
	[[ "$output" == *"--include-empty"* ]]
	[[ "$output" == *"Show zero-size project artifact directories"* ]]
}

@test "pe purge: accepts --debug flag" {
	if ! command -v gtimeout >/dev/null 2>&1 && ! command -v timeout >/dev/null 2>&1; then
		skip "gtimeout/timeout not available"
	fi
	...
	run bash -c "
        export HOME='$HOME'
        $timeout_cmd 10 '$PROJECT_ROOT/pear' purge --debug < /dev/null 2>&1 || true
    "
	true
}

@test "pe purge: accepts --dry-run flag" {
	if ! command -v gtimeout >/dev/null 2>&1 && ! command -v timeout >/dev/null 2>&1; then
		skip "gtimeout/timeout not available"
	fi
	...
	run bash -c "
        export HOME='$HOME'
        $timeout_cmd 10 '$PROJECT_ROOT/pear' purge --dry-run < /dev/null 2>&1 || true
    "

	[[ "$output" == *"DRY RUN MODE"* ]] || [[ "$output" == *"Dry run complete"* ]]
}

@test "pe purge: accepts --include-empty flag" {
	if ! command -v gtimeout >/dev/null 2>&1 && ! command -v timeout >/dev/null 2>&1; then
		skip "gtimeout/timeout not available"
	fi
	...
	run bash -c "
        export HOME='$HOME'
        $timeout_cmd 10 '$PROJECT_ROOT/pear' purge --include-empty --dry-run < /dev/null 2>&1
    "
...
@test "pe purge: creates cache directory for stats" {
	if ! command -v gtimeout >/dev/null 2>&1 && ! command -v timeout >/dev/null 2>&1; then
		skip "gtimeout/timeout not available"
	fi
	...
	bash -c "
        export HOME='$HOME'
        $timeout_cmd 10 '$PROJECT_ROOT/pear' purge < /dev/null 2>&1 || true
    "

	[ -d "$HOME/.cache/pear" ] || [ -d "${XDG_CACHE_HOME:-$HOME/.cache}/pear" ]
}
...
# selection. See https://github.com/RawSalmon69/pear-cli/issues/647
#
