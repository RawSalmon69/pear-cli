#!/bin/bash
# Pear - Application Protection
# System critical and data-protected application lists

set -euo pipefail

if [[ -n "${PEAR_APP_PROTECTION_LOADED:-}" ]]; then
    return 0
fi
readonly PEAR_APP_PROTECTION_LOADED=1

_PEAR_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
[[ -z "${PEAR_BASE_LOADED:-}" ]] && source "$_PEAR_CORE_DIR/base.sh"
...
source "$_PEAR_CORE_DIR/app_protection_data.sh"
...
# uninstaller instead of Pear's generic Trash/delete path.
...
#
# In uninstall mode (PEAR_UNINSTALL_MODE=1), only system-critical components are protected.
...
should_protect_path() {
    ...
    if [[ "$path" =~ /Library/Containers/([^/]+) ]] || [[ "$path" =~ /Library/Group\ Containers/([^/]+) ]]; then
        ...
        elif [[ "${PEAR_UNINSTALL_MODE:-0}" != "1" ]] && should_protect_data "$bundle_id"; then
            return 0
        fi
    fi
    ...
    case "$path" in
        */Library/Preferences/com.apple.dock.plist | */Library/Preferences/com.apple.finder.plist)
            return 0
            ;;
        # Protect Pear's own runtime logs so cleanup cannot delete its active log targets.
        */Library/Logs/pear | */Library/Logs/pear/ | */Library/Logs/pear/*)
            return 0
            ;;
    ...
    if [[ "$_container_cache_path" != "true" ]]; then
        if [[ "${PEAR_UNINSTALL_MODE:-0}" == "1" ]]; then
...
_pear_uninstall_lower() {
    printf '%s' "${1:-}" | tr '[:upper:]' '[:lower:]'
}

_pear_uninstall_is_common_app_name() {
    local lower_name
    lower_name=$(_pear_uninstall_lower "${1:-}")
    case "$lower_name" in
...
_pear_uninstall_vendor_product_tokens() {
    local bundle_id="${1:-}"
...
_pear_uninstall_name_variant_matches() {
...
find_vendor_nested_app_paths() {
    ...
    _pear_uninstall_is_common_app_name "$app_name" && return 0

    local token_pair
    token_pair=$(_pear_uninstall_vendor_product_tokens "$bundle_id" 2> /dev/null) || return 0
    ...
    vendor_lower=$(_pear_uninstall_lower "$vendor_token")
    product_lower=$(_pear_uninstall_lower "$product_token")
    app_lower=$(_pear_uninstall_lower "$app_name")
    nospace_lower=$(_pear_uninstall_lower "${app_name// /}")
    hyphen_lower=$(_pear_uninstall_lower "${app_name// /-}")
    underscore_lower=$(_pear_uninstall_lower "${app_name// /_}")
    ...
    for root in "$@"; do
        ...
        while IFS= read -r -d '' candidate; do
            ...
            parent_lower=$(_pear_uninstall_lower "$parent_base")
            [[ "$parent_lower" == "$vendor_lower" ]] || continue

            child_base="${candidate##*/}"
            child_lower=$(_pear_uninstall_lower "$child_base")
            if _pear_uninstall_name_variant_matches "$child_lower" \
...
find_shared_app_paths() {
    ...
    local product_token=""
    local token_pair
    if token_pair=$(_pear_uninstall_vendor_product_tokens "$bundle_id" 2> /dev/null); then
    ...
    for root in "$@"; do
        ...
        while IFS= read -r -d '' candidate; do
            base="${candidate##*/}"
            lower_base=$(_pear_uninstall_lower "$base")
            if _pear_uninstall_name_variant_matches "$lower_base" \
                "$app_lower" "$nospace_lower" "$hyphen_lower" "$underscore_lower" "$product_lower"; then
                printf '%s\n' "$candidate"
            fi
...
_pear_uninstall_embedded_bundle_ids() {
    local app_path="${1:-}"
    ...
    while IFS= read -r -d '' info; do
        ...
        pear_is_reverse_dns_bundle_id "$embedded_id" || continue
...
find_app_files() {
    ...
    if [[ "$bundle_id_valid" == "true" ]]; then
        ...
        [[ -d ~/Library/Preferences/ByHost ]] && while IFS= read -r -d '' pref; do
            if pear_name_starts_with_bundle_id_boundary "$pref" "$bundle_id"; then
                files_to_clean+=("$pref")
            fi
        ...
        if [[ -d ~/Library/Group\ Containers ]]; then
            while IFS= read -r -d '' container; do
                if pear_name_has_bundle_id_boundary "$container" "$bundle_id"; then
        ...
        for derived_root in "${derived_bundle_roots[@]}"; do
            ...
            while IFS= read -r -d '' derived_path; do
                pear_name_has_bundle_id_boundary "$derived_path" "$bundle_id" || continue
                already_added=false
    ...
    if [[ "$bundle_id_valid" == "true" && -n "$app_path" ]]; then
        ...
        done < <(_pear_uninstall_embedded_bundle_ids "$app_path" "$bundle_id")
    fi
    ...
    #
    # PEAR_UNINSTALL_SIBLING_SURVIVES=1 skips the whole heuristic section
    ...
    if [[ "${PEAR_UNINSTALL_SIBLING_SURVIVES:-0}" == "1" ]]; then
...
find_app_system_files() {
    ...
    if pear_is_reverse_dns_bundle_id "$bundle_id"; then
        [[ -d /Library/PrivilegedHelperTools ]] && while IFS= read -r -d '' helper; do
            if pear_name_starts_with_bundle_id_boundary "$helper" "$bundle_id"; then
                system_files+=("$helper")
            fi
        ...
        [[ -d /private/var/db/receipts ]] && while IFS= read -r -d '' receipt; do
            if pear_name_starts_with_bundle_id_boundary "$receipt" "$bundle_id"; then
    ...
    if ! _pear_uninstall_is_common_app_name "$app_name"; then
        local name_variant
    ...
    if [[ ${#helper_name_variants[@]} -gt 0 && -d /Library/PrivilegedHelperTools ]]; then
        while IFS= read -r -d '' helper; do
            ...
            helper_lower=$(_pear_uninstall_lower "$helper_name")
            if _pear_uninstall_name_variant_matches "$helper_lower" "${helper_name_variants[@]}"; then
                system_files+=("$helper")
            fi
        done < <(command find /Library/PrivilegedHelperTools -maxdepth 1 -print0 2> /dev/null)
    fi
...
find_app_receipt_files() {
    ...
    if [[ -d /private/var/db/receipts ]]; then
        while IFS= read -r -d '' bom; do
            if pear_name_starts_with_bundle_id_boundary "$bom" "$bundle_id"; then
                bom_files+=("$bom")
            fi
...
receipt_payload_path_is_allowlisted() {
    ...
    case "$clean_path" in
        ...
        /Library/PrivilegedHelperTools/*)
            pear_name_starts_with_bundle_id_boundary "$base" "$bundle_id"
            return
            ;;
...
force_kill_app() {
    ...
    if [[ "${PEAR_TEST_MODE:-0}" != "1" && "${PEAR_TEST_NO_AUTH:-0}" != "1" ]] &&
        ...
        run_with_timeout "$PEAR_TIMEOUT_SHORT_QUERY_SEC" osascript -e "tell application $quit_target to quit" > /dev/null 2>&1 &
        local quit_pid=$!
