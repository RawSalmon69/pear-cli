...
echo -e "${BLUE}=== Pear Check, ${MODE} ===${NC}\n"

SHELL_FILES=$(find . -type f \( -name "*.sh" -o -name "pear" \) \
    -not -path "./.git/*" \
...
if [[ "$MODE" == "format" ]]; then
    ...
    if command -v goimports > /dev/null 2>&1; then
        ...
        goimports -w -local github.com/RawSalmon69/pear-cli ./cmd ./internal
...
if command -v shellcheck > /dev/null 2>&1; then
    if shellcheck pear install.sh bin/*.sh lib/*/*.sh scripts/*.sh; then
...
if ! bash -n pear; then
    echo -e "${RED}${ICON_ERROR} Syntax check failed, pear${NC}\n"
    exit 1
fi
