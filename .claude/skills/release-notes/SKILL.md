---
name: release-notes
description: Publish curated release notes for a Pear `V<version>` tag. Encodes the compact bilingual format, the gh release edit (not create) flow, reporter/contributor thanks, and the six-reaction set. User-only because publishing is a side effect that touches the public release page.
...
6. **Verify release exists**. `gh release view V<version> --repo RawSalmon69/pear-cli --json id,name` should return non-empty. If it doesn't, the workflow hasn't finished, wait, don't `gh release create`.
...
- `grep '^VERSION=' pear` matches `<version>`.
- `SECURITY_AUDIT.md` opening line reflects the new version and date.
- `./scripts/check.sh --format` clean.
- `PEAR_TEST_NO_AUTH=1 PEAR_TEST_JOBS=2 BATS_FORMATTER=tap ./scripts/test.sh` exits 0.
...
`gh release view V1.45.0 --repo RawSalmon69/pear-cli --json body --jq .body`. (Do not copy older pages such as V1.43.1 or V1.44.x; their body h1 drifted and was rejected.)

Structure:

```
<div align="center">
  <img src="https://cdn.tw93.fun/pic/cole.png" alt="Pear Logo" width="120" height="120" style="border-radius:50%" />
...
Prefer a GUI? Try [Pear Mac App](https://github.com/RawSalmon69/pear-cli). The CLI stays free and open source.
```

No `---` separators between sections, and no trailing repository link; the published pages end on the Pear Mac App line.

### Format rules (all are documented bugs that have shipped before)

- **Body h1 is just `Pear`**. Version, codename, and emoji live only in the `--title` argument (`V<version> <CodeName> <emoji>`); repeating them in the body header is redundant and has been explicitly rejected before.
...
- **Verify every command mentioned in the notes actually exists in HEAD**. AGENTS.md cites `pe check / pe doctor` as a case where a removed command nearly shipped as a "feature".
- **Keep the Pear Mac App cross-link only if it matches the current release style**. Do not turn it into a sales block.
...
```bash
gh release edit V<version> --repo RawSalmon69/pear-cli \
...
- `gh release view V<version> --repo RawSalmon69/pear-cli --web` (open in browser) so the user can eyeball it.
...
- `scripts/sponsors.sh` - legacy helper for fetching recent sponsors. Do not use it in Pear release notes unless the user explicitly asks for sponsor names.
