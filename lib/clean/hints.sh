#!/bin/bash
# Hint notices used by `pe clean` (non-destructive guidance only).

set -euo pipefail

pear_hints_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1090
source "$pear_hints_dir/purge_shared.sh"

# Quick reminder probe for project build artifacts handled by `pe purge`.
# Designed to be very fast: shallow directory checks only, no deep find scans.
# shellcheck disable=SC2329
load_quick_purge_hint_paths() {
    local config_file="$HOME/.config/pear/purge_paths"
    local -a paths=()

    while IFS= read -r line; do
        [[ -n "$line" ]] && paths+=("$line")
    done < <(pear_purge_read_paths_config "$config_file")

    if [[ ${#paths[@]} -eq 0 ]]; then
        paths=("${PEAR_PURGE_DEFAULT_SEARCH_PATHS[@]}")
    fi
...
hint_collect_child_dirs_with_timeout() {
    ...
    # paths. Slow/cloud-backed roots are skipped so `pe clean` never appears
...
hint_collect_installed_gui_app_match_texts() {
    ...
    for app_root in "${app_roots[@]}"; do
        ...
        done < <(run_with_timeout "$PEAR_TIMEOUT_QUICK_DETECT_SEC" find "$app_root" -maxdepth 2 -name "*.app" -print0 2> /dev/null || true)
    done
...
probe_project_artifact_hints() {
    ...
    local hint_budget_seconds="${PEAR_TIMEOUT_HINT_SCAN_SEC:-15}"
    ...
    done < <(pear_purge_quick_hint_target_names)
...
readonly ORPHAN_DOTDIR_KNOWN_SAFE=(
    ...
    # Language tools (Pear handles their caches separately)
...
_dotdir_owner_collect_tokens() {
    ...
    for root in "${_PEAR_DOTDIR_OWNER_APP_ROOTS[@]}"; do
        [[ -d "$root" ]] || continue
    ...
    if command -v brew > /dev/null 2>&1; then
        local cask_list=""
        cask_list=$(HOMEBREW_NO_ENV_HINTS=1 run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" brew list --cask 2> /dev/null) || true
        if [[ -n "$cask_list" ]]; then
...
dotdir_has_owning_gui_app() {
    ...
    local cache_dir="$HOME/.cache/pear"
...
show_orphan_dotdir_hint_notice() {
    local max_hits=5
    local age_days="${PEAR_DOTDIR_ORPHAN_AGE_DAYS:-60}"
    local now
    now=$(date +%s)
    ...
    while IFS= read -r dotdir; do
        ...
        if [[ -d "$HOME/Library/LaunchAgents" ]]; then
            if run_with_timeout "$PEAR_TIMEOUT_QUICK_DETECT_SEC" grep -rlq "$basename" "$HOME/Library/LaunchAgents/" 2> /dev/null; then
                continue
            fi
        fi
