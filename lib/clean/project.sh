...
readonly PURGE_TARGETS=("${PEAR_PURGE_TARGETS[@]}")
...
readonly DEFAULT_PURGE_SEARCH_PATHS=("${PEAR_PURGE_DEFAULT_SEARCH_PATHS[@]}")

# Config file for custom purge paths.
readonly PURGE_CONFIG_FILE="$HOME/.config/pear/purge_paths"
...
readonly MONOREPO_INDICATORS=("${PEAR_PURGE_MONOREPO_INDICATORS[@]}")
readonly PROJECT_INDICATORS=("${PEAR_PURGE_PROJECT_INDICATORS[@]}")
...
discover_project_dirs() {
    ...
    for path in "${DEFAULT_PURGE_SEARCH_PATHS[@]}"; do
        if [[ -d "$path" ]]; then
            ...
            discovered+=("$(pear_purge_resolve_path_case "$path")")
        fi
    done
    ...
    for dir in "$HOME"/*/; do
        ...
        dir=$(pear_purge_resolve_path_case "$dir")
...
write_purge_config() {
    ...
    local tmp_file
    tmp_file=$(mktemp_file "pear-purge-paths") || return 1
...
EOF
    ...
    # from `pe purge --paths` passes only the header with no paths).
    if [[ ${#paths[@]} -gt 0 ]]; then
...
save_discovered_paths() {
    local -a paths=("$@")
    write_purge_config "# Pear Purge Paths - Auto-discovered project directories
# Edit this file to customize, or run: pe purge --paths
...
load_purge_config() {
    ...
    done < <(pear_purge_read_paths_config "$PURGE_CONFIG_FILE")
...
scan_purge_targets() {
    ...
    local stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/pear"
    ...
    emit_valid_cachedir_tag_dirs() {
        while IFS= read -r tag_file; do
            ...
            if [[ -n "$cache_dir" ]] && pear_dir_has_cachedir_tag "$cache_dir"; then
                printf '%s\n' "$cache_dir"
            fi
        done
    }
    ...
    # Allow forcing find via PE_USE_FIND environment variable
    if [[ "${PE_USE_FIND:-0}" == "1" ]]; then
        debug_log "PE_USE_FIND=1: Forcing find instead of fd"
        use_find=true
        ...
        local _scan_timeout="${PE_PURGE_SCAN_TIMEOUT_SEC:-60}"
        if run_with_timeout "$_scan_timeout" fd "${fd_args[@]}" "$pattern" "$search_path" 2> /dev/null > "$output_file.raw"; then
            run_with_timeout "$_scan_timeout" fd "${fd_tag_args[@]}" "^${PEAR_CACHEDIR_TAG_NAME}$" "$search_path" \
    ...
    if [[ "$use_find" == "true" ]]; then
        ...
        run_with_timeout "$_scan_timeout" find "$search_path" -mindepth "$cachedir_tag_min_depth" -maxdepth "$cachedir_tag_max_depth" \
            \( "${prune_expr[@]}" \) -prune -o \
            -type f -name "$PEAR_CACHEDIR_TAG_NAME" -print \
            2> /dev/null | emit_valid_cachedir_tag_dirs >> "$output_file.raw" || true
...
get_dir_size_kb() {
    ...
    local timeout_seconds="${PE_PURGE_SIZE_TIMEOUT_SEC:-15}"
...
clean_project_artifacts() {
    ...
    local dedupe_output
    dedupe_output=$(mktemp_file "pear-purge-dedupe") || return 1
    ...
    for item in "${safe_to_clean[@]}"; do
        ...
        if [[ "$size_raw" == "TIMEOUT" ]]; then
            ...
            size_kb="$size_raw"
            if [[ $size_kb -eq 0 && "${PEAR_PURGE_INCLUDE_EMPTY:-0}" != "1" ]]; then
                continue
            fi
    ...
    local dry_run_mode="${PEAR_DRY_RUN:-0}"
    for idx in "${selected_indices[@]}"; do
