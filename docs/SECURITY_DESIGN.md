# Pear Security Design

This document describes the safety mechanisms that prevent pear from
destroying data it shouldn't. It is written for reviewers, contributors,
and anyone evaluating pear for production use.
...
Pear is a user-invoked CLI that performs three classes of destructive
...
3. **Trash routing** — move user-selected files in `pe analyze` to Trash.

We assume:
- The invoking user has shell access and runs pear intentionally.
...
- A user who runs `sudo pear` with malicious flags they typed in deliberately.
- A compromised macOS host where SIP is disabled and `/System` is writable.
- Supply-chain compromise of the pear binary itself (covered separately
...
Every removal in pear funnels through `pear_delete` /
...
`pe analyze` and `pe clean`'s ad-hoc paths route deletions to the macOS
...
deletion requires explicit `--permanent` or going through `pe clean`'s
...
- `PEAR_DRY_RUN=1` — every safe-remove logs what it would do and
...
- `PEAR_TEST_NO_AUTH=1` — refuses to call `sudo`, `osascript`,
...
- **No code signing of the cleanup config.** We rely on filesystem
  ...
  edit `pear` itself; the threat model says we don't defend that.
- **No anti-rollback.** A user who restores an old pear binary or
...
- **No protection for arbitrary user paths.** `~/Documents/important`
  ...
  cleanup targets; pear only guarantees system integrity.
- **No telemetry.** We never report what was scanned, deleted, or
  attempted. Mistakes are diagnosed locally via `~/.cache/pear/`
  operation logs (path is `PEAR_OPLOG_PATH` overridable;
  `PE_NO_OPLOG=1` disables entirely).
...
Last reviewed: 2026-05-21 (pear V1.39.0).
