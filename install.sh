#!/bin/bash
# Pear - Installer for manual installs.
...
ensure_sudo_ready() {
    if [[ "${PEAR_TEST_MODE:-0}" == "1" || "${PEAR_TEST_NO_AUTH:-0}" == "1" ]]; then
        log_error "Admin access required, blocked in test mode"
        return 1
    fi

    if [[ "${PEAR_ASSUME_SUDO_AUTH:-0}" == "1" ]]; then
        sudo -n -v
        return
    fi
...
resolve_source_dir() {
    if [[ -n "$SOURCE_DIR" && -d "$SOURCE_DIR" && -f "$SOURCE_DIR/pear" ]]; then
        return 0
    fi
    ...
    if [[ -n "${BASH_SOURCE[0]:-}" && -f "${BASH_SOURCE[0]}" ]]; then
        ...
        if [[ -f "$script_dir/pear" ]]; then
            SOURCE_DIR="$script_dir"
            return 0
        fi
    fi

    if [[ -n "${CLEAN_SOURCE_DIR:-}" && -d "$CLEAN_SOURCE_DIR" && -f "$CLEAN_SOURCE_DIR/pear" ]]; then
    ...
    local url="https://github.com/RawSalmon69/pear-cli/archive/refs/heads/main.tar.gz"

    if [[ "$branch" == "dev" ]]; then
        url="https://github.com/RawSalmon69/pear-cli/archive/refs/heads/dev.tar.gz"
    elif [[ "$branch" != "main" ]]; then
        url="https://github.com/RawSalmon69/pear-cli/archive/refs/tags/${branch}.tar.gz"
    fi

    start_line_spinner "Fetching Pear source, ${branch}..."
    if command -v curl > /dev/null 2>&1; then
        if curl -fsSL --connect-timeout 10 --max-time 60 -o "$tmp/pear.tar.gz" "$url" 2> /dev/null; then
            if tar -xzf "$tmp/pear.tar.gz" -C "$tmp" 2> /dev/null; then
                stop_line_spinner

                local extracted_dir
                extracted_dir=$(find "$tmp" -mindepth 1 -maxdepth 1 -type d | head -n 1)

                if [[ -n "$extracted_dir" && -f "$extracted_dir/pear" ]]; then
    ...
    start_line_spinner "Cloning Pear source..."
    if command -v git > /dev/null 2>&1; then
        ...
        if git clone "${git_args[@]}" https://github.com/RawSalmon69/pear-cli.git "$tmp/pear" > /dev/null 2>&1; then
            stop_line_spinner
            SOURCE_DIR="$tmp/pear"
            return 0
        fi
...
# Version helpers
get_source_version() {
    local source_pear="$SOURCE_DIR/pear"
    if [[ -f "$source_pear" ]]; then
        sed -n 's/^VERSION="\(.*\)"$/\1/p' "$source_pear" | head -n1
    fi
}
...
get_latest_release_tag_from_git() {
    ...
    git ls-remote --tags --refs https://github.com/RawSalmon69/pear-cli.git 2> /dev/null |
        awk -F/ '{print $NF}' |
        grep -E '^V[0-9]' |
        sort -V |
        tail -n 1
...
release_checksums_url() {
    local tag="$1"
    printf 'https://github.com/RawSalmon69/pear-cli/releases/download/%s/SHA256SUMS\n' "$tag"
}
...
verify_release_asset_checksum() {
    ...
    checksums_file="$(mktemp "${TMPDIR:-/tmp}/pear-checksums.XXXXXX")" || return 1
    ...
    if download_release_checksums "$tag" "$checksums_file" > /dev/null 2>&1; then
        ...
        if [[ "$attestation_status" -eq 2 && "${PEAR_REQUIRE_ATTESTATION:-0}" == "1" ]]; then
            log_error "PEAR_REQUIRE_ATTESTATION=1 set but gh CLI unavailable or unauthenticated"
            rm -f "$checksums_file"
            return 1
        fi
...
get_installed_version() {
    local binary="$INSTALL_DIR/pear"
    if [[ -x "$binary" ]]; then
        local version
        version=$("$binary" --version 2> /dev/null | awk '/Pear version/ {print $NF; exit}')
        if [[ -n "$version" ]]; then
            echo "$version"
        else
...
resolve_install_channel() {
    ...
    if [[ "${PEAR_EDGE_INSTALL:-}" == "true" ]]; then
        printf 'nightly\n'
        return 0
    fi
...
parse_args() {
    ...
    for i in "${!args[@]}"; do
        ...
        case "$token" in
            latest | main)
                export PEAR_VERSION="main"
                export PEAR_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            dev)
                export PEAR_VERSION="dev"
                export PEAR_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            [0-9]* | V[0-9]* | v[0-9]*)
                export PEAR_VERSION="$token"
                version_token="$token"
                unset 'args[$i]'
                ;;
            *)
...
check_requirements() {
    ...
    if command -v brew > /dev/null 2>&1 && brew list pear > /dev/null 2>&1; then
        local pear_path
        pear_path=$(command -v pear 2> /dev/null || true)
        local is_homebrew_binary=false

        if [[ -n "$pear_path" && -L "$pear_path" ]]; then
            if readlink "$pear_path" | grep -q "Cellar/pear"; then
        ...
        if [[ "$is_homebrew_binary" == "true" ]]; then
            ...
            echo -e "${YELLOW}Pear is installed via Homebrew${NC}"
            echo ""
            echo "Choose one:"
            echo -e "  1. Update via Homebrew: ${GREEN}brew upgrade pear${NC}"
            echo -e "  2. Switch to manual: ${GREEN}brew uninstall --force pear${NC} then re-run this"
            echo ""
            exit 1
        else
            log_warning "Cleaning up stale Homebrew installation..."
            brew uninstall --force pear > /dev/null 2>&1 || true
        fi
    fi
...
download_binary() {
    ...
    local url="https://github.com/RawSalmon69/pear-cli/releases/download/${release_tag}/${asset_name}"
    ...
    if [[ -n "$fallback_tag" && "$fallback_tag" != "$release_tag" ]]; then
        local fallback_url="https://github.com/RawSalmon69/pear-cli/releases/download/${fallback_tag}/${asset_name}"
        if [[ -t 1 ]]; then
...
install_files() {
    ...
    if [[ -f "$SOURCE_DIR/pear" ]]; then
        if [[ "$source_dir_abs" != "$install_dir_abs" ]]; then
            ...
            maybe_sudo cp "$SOURCE_DIR/pear" "$INSTALL_DIR/pear.new"
            maybe_sudo chmod +x "$INSTALL_DIR/pear.new"
            maybe_sudo mv -f "$INSTALL_DIR/pear.new" "$INSTALL_DIR/pear"

            log_success "Installed pear to $INSTALL_DIR"
        fi
    else
        log_error "pear executable not found in ${SOURCE_DIR:-unknown}"
        exit 1
    fi

    if [[ -f "$SOURCE_DIR/pe" ]]; then
        if [[ "$source_dir_abs" == "$install_dir_abs" ]]; then
            log_success "pe alias already present"
        else
            maybe_sudo cp "$SOURCE_DIR/pe" "$INSTALL_DIR/pe.new"
            maybe_sudo chmod +x "$INSTALL_DIR/pe.new"
            maybe_sudo mv -f "$INSTALL_DIR/pe.new" "$INSTALL_DIR/pe"
            log_success "Installed pe alias"
        fi
    fi
    ...
    if [[ "$source_dir_abs" != "$install_dir_abs" ]]; then
        ...
        maybe_sudo /usr/bin/sed -i '' "s|SCRIPT_DIR=.*|SCRIPT_DIR=\"$CONFIG_DIR\"|" "$INSTALL_DIR/pear"
    fi
...
verify_installation() {
    ...
    if [[ -x "$INSTALL_DIR/pear" ]] && [[ -f "$CONFIG_DIR/lib/core/common.sh" ]]; then

        if "$INSTALL_DIR/pear" --help > /dev/null 2>&1; then
            return 0
        else
            log_warning "Pear command installed but may not be working properly"
        fi
    else
...
setup_path() {
    ...
    if [[ "$INSTALL_DIR" != "/usr/local/bin" ]]; then
        ...
        echo ""
        echo "To use pear from anywhere, add this line to your shell profile:"
...
print_usage_summary() {
    ...
    local message="Pear ${action} successfully"
    ...
    echo "Usage:"
    if [[ ":$PATH:" == *":$INSTALL_DIR:"* ]]; then
        echo "  pe                           # Interactive menu"
        echo "  pe clean                     # Deep cleanup"
        echo "  pe uninstall                 # Remove apps + leftovers"
        echo "  pe optimize                  # Check and maintain system"
        echo "  pe analyze                   # Explore disk usage"
        echo "  pe status                    # Monitor system health"
        echo "  pe touchid                   # Configure Touch ID for sudo"
        echo "  pe update                    # Update to latest version"
        echo "  pe --help                    # Show all commands"
    else
        echo "  $INSTALL_DIR/pe                           # Interactive menu"
        echo "  $INSTALL_DIR/pe clean                     # Deep cleanup"
        echo "  $INSTALL_DIR/pe uninstall                 # Remove apps + leftovers"
        echo "  $INSTALL_DIR/pe optimize                  # Check and maintain system"
        echo "  $INSTALL_DIR/pe analyze                   # Explore disk usage"
        echo "  $INSTALL_DIR/pe status                    # Monitor system health"
        echo "  $INSTALL_DIR/pe touchid                   # Configure Touch ID for sudo"
        echo "  $INSTALL_DIR/pe update                    # Update to latest version"
        echo "  $INSTALL_DIR/pe --help                    # Show all commands"
    fi
    echo ""
}
...
perform_install() {
    ...
    if [[ "${PEAR_EDGE_INSTALL:-}" == "true" ]]; then
        ...
        echo ""
        local branch_name="${PEAR_VERSION:-main}"
        log_warning "Edge version installed on ${branch_name} branch"
        log_info "This is a testing version; use 'pe update' to switch to stable"
    fi
...
perform_update() {
    ...
    if command -v brew > /dev/null 2>&1 && brew list pear > /dev/null 2>&1; then
        ...
        if [[ -f "$SOURCE_DIR/lib/core/common.sh" ]]; then
            ...
            log_error "Cannot update Homebrew-managed Pear without full installation"
            echo ""
            echo "Please update via Homebrew:"
            echo -e "  ${GREEN}brew upgrade pear${NC}"
            exit 1
        fi
        exit 0
    fi
    ...
    if [[ -z "$installed_version" ]]; then
        log_warning "Pear is not currently installed in $INSTALL_DIR. Running fresh installation."
        perform_install
        return
    fi
    ...
    if [[ -z "$target_version" ]]; then
        log_error "Unable to determine the latest Pear version."
        exit 1
    fi
