...
@test "safe_sudo_remove never opens an interactive sudo prompt" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" PEAR_TEST_MODE=0 PEAR_TEST_NO_AUTH=0 bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
cat "$TRACE"
echo "--OPLOG--"
cat "$HOME/Library/Logs/pear/operations.log" 2> /dev/null || true
exit 0
SCRIPT
    chmod +x "$script"
...
SCRIPT
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
        PE_NO_OPLOG=1 PEAR_TEST_MODE=0 PEAR_TEST_NO_AUTH=0 bash --noprofile --norc "$script"
    ...
    if [ "$status" -ne 0 ]; then
        ...
        env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" TARGET_DIR="$target_dir" \
            PE_NO_OPLOG=1 PEAR_TEST_MODE=0 PEAR_TEST_NO_AUTH=0 \
...
@test "PEAR_* constants are defined" {
    run bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$PEAR_TEMP_FILE_AGE_DAYS"
    [ "$status" -eq 0 ]
    [ "$output" = "7" ]

    run bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$PEAR_MAX_PARALLEL_JOBS"
    [ "$status" -eq 0 ]
    [ "$output" = "15" ]

    run bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$PEAR_TM_BACKUP_SAFE_HOURS"
    [ "$status" -eq 0 ]
    [ "$output" = "48" ]
}
