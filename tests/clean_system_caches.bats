...
setup() {
    ...
    rm -f "$HOME/.cache/pear/permissions_granted"
}
...
@test "check_tcc_permissions skips in non-interactive mode" {
    ...
    [ "$status" -eq 0 ]
    [[ ! -f "$HOME/.cache/pear/permissions_granted" ]]
}

@test "check_tcc_permissions skips when permissions already granted" {
    mkdir -p "$HOME/.cache/pear"
    touch "$HOME/.cache/pear/permissions_granted"
...
set -euo pipefail
printf '%s\n' "$HOME/CustomProjects" > "$HOME/.config/pear/purge_paths"
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/caches.sh"
run_with_timeout() { shift; "$@"; }
safe_clean() { echo "$2|$1"; }
...
EOF
    ...
    rm -rf "$HOME/CustomProjects" "$HOME/.config/pear" "$fake_bin" "$find_log"
}
...
@test "discover_project_cache_roots dedupes aliased roots by filesystem identity" {
    ...
    printf '%s\n' "$HOME/Code" > "$HOME/.config/pear/purge_paths"
...
@test "clean_project_caches skips stalled root scans" {
    ...
    printf '%s\n' "$HOME/SlowProjects" > "$HOME/.config/pear/purge_paths"
...
source "$PROJECT_ROOT/lib/clean/caches.sh"
PE_TIMEOUT_BIN=""
PE_TIMEOUT_PERL_BIN="${PE_TIMEOUT_PERL_BIN:-$(command -v perl)}"
export PEAR_PROJECT_CACHE_DISCOVERY_TIMEOUT=0.5
export PEAR_PROJECT_CACHE_SCAN_TIMEOUT=0.5
SECONDS=0
clean_project_caches
...
EOF
    ...
    rm -rf "$HOME/.config/pear" "$HOME/SlowProjects" "$fake_bin"
}
