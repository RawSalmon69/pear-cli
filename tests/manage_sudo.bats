...
@test "stop_sudo_session cleans up keepalive process" {
    export PEAR_SUDO_KEEPALIVE_PID="99999"

    run bash -c "export PEAR_SUDO_KEEPALIVE_PID=99999; source '$PROJECT_ROOT/lib/core/common.sh'; source '$PROJECT_ROOT/lib/core/sudo.sh'; stop_sudo_session"
    [ "$status" -eq 0 ]
}

@test "sudo manager initializes global state correctly" {
    result=$(bash -c "source '$PROJECT_ROOT/lib/core/common.sh'; source '$PROJECT_ROOT/lib/core/sudo.sh'; echo \$PEAR_SUDO_ESTABLISHED")
