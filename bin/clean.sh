...
WHITELIST_WARNINGS=()
if [[ -f "$PEAR_USER_HOME/.config/pear/whitelist" ]]; then
    while IFS= read -r line; do
        ...
        [[ "$line" == ~* ]] && line="${line/#~/$PEAR_USER_HOME}"
        line="${line//\$HOME/$PEAR_USER_HOME}"
        line="${line//\$\{HOME\}/$PEAR_USER_HOME}"
        if [[ "$line" =~ \.\. ]]; then
    ...
    done < "$PEAR_USER_HOME/.config/pear/whitelist"
else
...
expand_whitelist_patterns() {
    if [[ ${#WHITELIST_PATTERNS[@]} -gt 0 ]]; then
        ...
        for pattern in "${WHITELIST_PATTERNS[@]}"; do
            local expanded="${pattern/#\~/$PEAR_USER_HOME}"
...
register_dry_run_cleanup_target() {
    local path="$1"
    local identity
    identity=$(pear_path_identity "$path")

    if [[ ${#DRY_RUN_SEEN_IDENTITIES[@]} -gt 0 ]] && pear_identity_in_list "$identity" "${DRY_RUN_SEEN_IDENTITIES[@]}"; then
        return 1
    fi
...
read_clean_sudo_choice() {
    ...
    local previous_force_char="${PEAR_READ_KEY_FORCE_CHAR:-}"
    if [[ ${PEAR_READ_KEY_FORCE_CHAR+x} ]]; then
        had_force_char=true
    fi
    ...
    if [[ "$had_force_char" == "true" ]]; then
        export PEAR_READ_KEY_FORCE_CHAR="$previous_force_char"
    else
        unset PEAR_READ_KEY_FORCE_CHAR
    fi
...
safe_clean() {
    ...
    local permission_start=${PEAR_PERMISSION_DENIED_COUNT:-0}
    ...
    if [[ "${PE_DEBUG:-}" == "1" && ${#existing_paths[@]} -gt 0 ]]; then
        ...
        if [[ $dir_count -lt 5 && ${#existing_paths[@]} -gt 20 ]]; then
            ...
            if [[ ${#existing_paths[@]} -gt 0 ]]; then
                for path in "${existing_paths[@]}"; do
                    ...
                    if ((${#pids[@]} >= PEAR_MAX_PARALLEL_JOBS)); then
    ...
    local permission_end=${PEAR_PERMISSION_DENIED_COUNT:-0}
...
start_cleanup() {
    ...
    export PEAR_CURRENT_COMMAND="clean"
...
# 1. Copy any path below to ~/.config/pear/whitelist
# 2. Run: pe clean --whitelist
#
# Example:
...
EOF
        ...
        if adopt_sudo_session; then
            ...
            SYSTEM_CLEAN=false
            echo -e "${GRAY}${ICON_WARNING} System caches need sudo, run ${NC}sudo -v && pe clean --dry-run${GRAY} for full preview${NC}"
            echo ""
        fi
        return
    fi
...
perform_cleanup() {
    ...
    if [[ -z "$EXTERNAL_VOLUME_TARGET" && "${PEAR_TEST_MODE:-0}" == "1" ]]; then
    ...
    if [[ $total_size_cleaned -gt 0 ]]; then
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            ...
            if ((total_size_cleaned >= PEAR_ONE_GIB_KB)); then
                local freed_gb=$((total_size_cleaned / PEAR_ONE_GIB_KB))
...
run_with_shell_timeout() {
    ...
    PE_TIMEOUT_BIN="" PE_TIMEOUT_PERL_BIN="" run_with_timeout "$duration" "$@"
}
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            "--select" | "--categories" | "--exclude")
                ...
                echo "Use 'pe clean --dry-run' to preview cleanup and 'pe clean --whitelist' to protect paths." >&2
                exit 1
                ;;
            -*)
                echo "Unknown option for pe clean: $1" >&2
                echo "Run 'pe clean --help' for usage." >&2
                exit 1
                ;;
            *)
                echo "Unexpected argument for pe clean: $1" >&2
                echo "Run 'pe clean --help' for usage." >&2
                exit 1
                ;;
        esac
