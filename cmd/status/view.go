...
import (
	...
	"github.com/RawSalmon69/pear-cli/internal/units"
)
...
func renderHeader(m MetricsSnapshot, errMsg string, animFrame int, termWidth int, catHidden bool) (string, string) {
	...
	// Show cat unless hidden - render pear centered below header
	var pear string
	if !catHidden {
		pear = getCatFrame(animFrame, termWidth)
	}
	...
	if errMsg != "" {
		...
		return lipgloss.JoinVertical(lipgloss.Left, headerLine, "", pear, dangerStyle.Render("ERROR: "+errMsg)), ""
	}
	if pear == "" {
		return headerLine, ""
	}
	return headerLine, pear
}
