...
[[ -z "${PEAR_COMMON_LOADED:-}" ]] && source "$SCRIPT_DIR/lib/core/common.sh"
...
# stays in bootout_login_item_helpers via pear_is_reverse_dns_bundle_id.
...
# tests exercise the real branch with PEAR_TEST_MODE=0 and a launchctl mock.
_uninstall_background_job_loaded() {
    ...
    while IFS= read -r label; do
        [[ -n "$label" ]] || continue
        pear_is_reverse_dns_bundle_id "$label" || continue
...
discover_login_item_helper_bundle_ids() {
    ...
    while IFS= read -r -d '' helper; do
        ...
        if pear_is_reverse_dns_bundle_id "$bundle_id"; then
            printf '%s\n' "$bundle_id"
        fi
...
bootout_login_item_helpers() {
    ...
    if is_uninstall_dry_run || [[ "${PEAR_TEST_MODE:-0}" == "1" || "${PEAR_TEST_NO_AUTH:-0}" == "1" ]]; then
    ...
    while IFS= read -r helper_id; do
        ...
        pear_is_reverse_dns_bundle_id "$helper_id" || continue
        ...
        run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" launchctl bootout "gui/$uid/$helper_id" > /dev/null 2>&1 || true
    done <<< "$helper_ids"
}
...
unload_launch_plist() {
    ...
    if [[ "$needs_sudo" == "true" ]]; then
        run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" sudo launchctl unload "$plist" > /dev/null 2>&1 || true
    else
        run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" launchctl unload "$plist" > /dev/null 2>&1 || true
    fi
}
...
stop_launch_services() {
    ...
    elif ! pear_is_reverse_dns_bundle_id "$bundle_id"; then
    ...
    if [[ "$bundle_id_usable" == "true" && "$has_system_files" == "true" && "${PEAR_TEST_MODE:-0}" != "1" && "${PEAR_TEST_NO_AUTH:-0}" != "1" ]]; then
    ...
    if [[ -n "$app_path" ]]; then
        ...
        if [[ "$has_system_files" == "true" && "${PEAR_TEST_MODE:-0}" != "1" && "${PEAR_TEST_NO_AUTH:-0}" != "1" ]]; then
            if [[ -d /Library/LaunchAgents ]]; then
                while IFS= read -r -d '' plist; do
                    grep -qF -- "$app_path" "$plist" 2> /dev/null || continue
                    unload_launch_plist "$plist" "true"
...
refresh_launch_services_after_uninstall() {
    ...
    run_with_timeout "$PEAR_TIMEOUT_PKG_LIST_SEC" "$lsregister" -gc > /dev/null 2>&1 || true
    ...
    if [[ $success -eq 124 ]]; then
        ...
        run_with_timeout "$PEAR_TIMEOUT_PKG_LIST_SEC" "$lsregister" -r -f -domain local -domain user > /dev/null 2>&1
        success=$?
...
# Performance: when PEAR_DELETE_MODE=trash and the batch is sudo-free and
...
# terminal" reported during `pe uninstall` on apps with many leftovers.
remove_file_list() {
    local file_list="$1"
    ...
    while IFS= read -r file; do
        ...
        # Symlinks and sudo-required paths stay on the per-file pear_delete
    ...
    if [[ ${#trash_batch[@]} -gt 0 ]]; then
        if _pear_move_to_trash_batch "${trash_batch[@]}"; then
            local _bp _bsize
            for _bp in "${trash_batch[@]}"; do
                _bsize="unknown"
                _pear_delete_log "trash" "$_bsize" "ok" "$_bp"
                log_operation "${PEAR_CURRENT_COMMAND:-uninstall}" "TRASHED" "$_bp" "batch"
            done
            count=$((count + ${#trash_batch[@]}))
        else
            # Batch failed wholesale: route each path through pear_delete so
...
_batch_scan_app_details() {
    ...
    for selected_app in "${selected_apps[@]}"; do
        ...
        if [[ -n "$discovery_app_name" ]]; then
            ...
            related_files=$(PEAR_UNINSTALL_SIBLING_SURVIVES="$sibling_survives" find_app_files "$bundle_id" "$discovery_app_name" "$app_path" || true)
...
_batch_preview_and_confirm() {
    ...
    # does not work reliably under Pear's timed non-interactive execution path.
    if [[ "${PEAR_DRY_RUN:-0}" != "1" ]] &&
    ...
    for detail in "${app_details[@]}"; do
        ...
        if [[ -z "$reason" ]]; then
            if [[ "$is_brew_cask" == "true" && -n "$cask_name" ]]; then
                ...
                if brew_uninstall_cask "$cask_name" "$app_path" "$cask_zap_mode"; then
                    ...
                    # where brew still reports the app as installed after Pear
                    ...
                    if [[ $cask_state -eq 1 ]]; then
                        if ! pear_delete "$app_path" "$needs_sudo"; then
                ...
                if [[ -L "$app_path" ]]; then
                    ...
                    if [[ -n "$link_target" ]]; then
                        ...
                        case "$resolved_target" in
                            /System/* | /usr/bin/* | /usr/lib/* | /bin/* | /sbin/* | /private/etc/*)
                                ...
                                ;;
                            *)
                                if ! pear_delete "$app_path" "true"; then
                    ...
                    if is_uninstall_dry_run; then
                        if ! pear_delete "$app_path" "false"; then
                            reason="dry-run path validation failed"
                        fi
                    else
                        local ret=0
                        pear_delete "$app_path" "true" || ret=$?
                        if [[ $ret -ne 0 ]]; then
                            local diagnosis
        ...
        if [[ -z "$reason" ]]; then
            ...
            if pear_is_reverse_dns_bundle_id "$bundle_id"; then
                ...
                # User-owned plists, so route through user-mode pear_delete to
                ...
                if [[ -d "$HOME/Library/Preferences/ByHost" ]]; then
                    while IFS= read -r -d '' plist_file; do
                        pear_delete "$plist_file" "false" || true
            ...
            if pear_is_reverse_dns_bundle_id "$bundle_id" && [[ -d /Library/SystemExtensions ]]; then
                local system_extension_path=""
                local has_bundle_system_extension=false
                while IFS= read -r -d '' system_extension_path; do
                    if pear_name_starts_with_bundle_id_boundary "$system_extension_path" "$bundle_id"; then
...
_batch_render_summary() {
    ...
    if [[ ${#local_network_warning_apps[@]} -gt 0 ]]; then
        ...
        summary_details+=("${GRAY}${ICON_SUBLIST}${NC} Pear does not reset ${GRAY}/Volumes/Data/Library/Preferences/com.apple.networkextension*.plist${NC}")
...
batch_uninstall_applications() {
    ...
    trap 'stop_inline_spinner 2>/dev/null; _cleanup_sudo_keepalive; unset PEAR_UNINSTALL_MODE; echo ""; _restore_uninstall_traps; _batch_interrupted=1; return 130' INT TERM
    ...
    if [[ $brew_apps_removed -gt 0 && "${PEAR_DRY_RUN:-0}" != "1" ]]; then
        (
            HOMEBREW_NO_ENV_HINTS=1 HOMEBREW_NO_AUTO_UPDATE=1 NONINTERACTIVE=1 \
                run_with_timeout "$PEAR_TIMEOUT_DISK_VERIFY_SEC" brew autoremove > /dev/null 2>&1 || true
        ) &
