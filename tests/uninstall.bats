...
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
pear_delete() { echo "PEAR_DELETE:$1"; return 0; }
...
unset PEAR_TEST_MODE PEAR_TEST_NO_AUTH
...
<plist version="1.0"><dict>
  <key>CFBundleExecutable</key><string>TestApp</string>
  <key>CFBundleIdentifier</key><string>com.example.TestApp&quot; to display dialog &quot;pear</string>
</dict></plist>
PLIST
...
@test "batch_uninstall_applications proceeds with deletion when force_kill_app fails" {
	# Reproduces the issue where uninstalling a still-running app (e.g. Pear.app
...
@test "bootout_login_item_helpers never touches the com.apple namespace" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_TEST_MODE=0 PEAR_TEST_NO_AUTH=0 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/uninstall/batch.sh"
...
PEAR_UNINSTALL_USER_LC_ALL=""
PEAR_UNINSTALL_USER_LANG=""
...
@test "remove_pear deletes manual binaries and caches" {
	mkdir -p "$HOME/.local/bin"
	touch "$HOME/.local/bin/pear"
	touch "$HOME/.local/bin/pe"
	mkdir -p "$HOME/.config/pear" "$HOME/.cache/pear" "$HOME/Library/Logs/pear"

	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PATH="/usr/bin:/bin" PEAR_TEST_MODE=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
rm() {
...
printf '\n' | "$PROJECT_ROOT/pear" remove
EOF

	[ "$status" -eq 0 ]
	[ ! -f "$HOME/.local/bin/pear" ]
	[ ! -f "$HOME/.local/bin/pe" ]
	[ ! -d "$HOME/.config/pear" ]
	[ ! -d "$HOME/.cache/pear" ]
	[ ! -d "$HOME/Library/Logs/pear" ]
}

@test "remove_pear dry-run keeps manual binaries and caches" {
	mkdir -p "$HOME/.local/bin"
	touch "$HOME/.local/bin/pear"
	touch "$HOME/.local/bin/pe"
	mkdir -p "$HOME/.config/pear" "$HOME/.cache/pear" "$HOME/Library/Logs/pear"
...
printf '\n' | "$PROJECT_ROOT/pear" remove --dry-run
EOF
...
@test "remove_pear test mode ignores PATH installs outside test HOME" {
	mkdir -p "$HOME/.local/bin" "$HOME/.config/pear" "$HOME/.cache/pear" "$HOME/Library/Logs/pear"
	touch "$HOME/.local/bin/pear"
	touch "$HOME/.local/bin/pe"

	fake_global_bin="$(mktemp -d "${BATS_TEST_DIRNAME}/tmp-remove-path.XXXXXX")"
	touch "$fake_global_bin/pear"
	touch "$fake_global_bin/pe"
...
EOF
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PATH="$fake_global_bin:/usr/bin:/bin" PEAR_TEST_MODE=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
export -f start_inline_spinner stop_inline_spinner
...
EOF
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"$HOME/.local/bin/pear"* ]]
	[[ "$output" == *"$HOME/.local/bin/pe"* ]]
	[[ "$output" != *"$fake_global_bin/pear"* ]]
	[[ "$output" != *"$fake_global_bin/pe"* ]]
	[[ "$output" != *"brew uninstall --force pear"* ]]
}
...
start_uninstall_interactive_screen() {
    export PEAR_ALT_SCREEN_ACTIVE=1
    export PEAR_MANAGED_ALT_SCREEN=1
    printf 'start\n' >> "$trace_file"
}
stop_uninstall_interactive_screen() {
    printf 'stop\n' >> "$trace_file"
    unset PEAR_ALT_SCREEN_ACTIVE PEAR_MANAGED_ALT_SCREEN
}
scan_applications() {
    printf 'scan\n' >> "$trace_file"
    printf '%s\n' "$app_cache_file"
...
select_apps_for_uninstall() {
    [[ "${PEAR_ALT_SCREEN_ACTIVE:-}" == "1" ]]
    [[ "${PEAR_MANAGED_ALT_SCREEN:-}" == "1" ]]
    printf 'select\n' >> "$trace_file"
    return 1
}
...
@test "scan_applications starts feedback before discovery and cleans no-app state" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_TEST_FORCE_SCAN_SPINNER=1 bash --noprofile --norc <<'INNER'
set -euo pipefail

trace_file="$HOME/scan-feedback-trace.log"
scan_temp="$HOME/scan-feedback-temp"

PEAR_UNINSTALL_META_CACHE_DIR="$HOME/.cache/pear"
PEAR_UNINSTALL_META_CACHE_FILE="$PEAR_UNINSTALL_META_CACHE_DIR/uninstall_app_metadata_v1"
PEAR_UNINSTALL_META_CACHE_LOCK="${PEAR_UNINSTALL_META_CACHE_FILE}.lock"
...
paginated_multi_select() {
    printf 'guard:%s\n' "${PEAR_MENU_IGNORE_INITIAL_ENTER:-unset}" >> "$trace_file"
    printf 'paginated\n' >> "$trace_file"
    PEAR_SELECTION_RESULT="0"
    return 0
}

select_apps_for_uninstall
[[ ${#selected_apps[@]} -eq 1 ]]
[[ -z "${PEAR_MENU_IGNORE_INITIAL_ENTER:-}" ]]
...
PEAR_SELECTION_RESULT=""
set +e
PEAR_MENU_IGNORE_INITIAL_ENTER=1 paginated_multi_select "Test Menu" "First App" > "$HOME/menu.out" 2> "$HOME/menu.err"
rc=$?
set -e

echo "rc=$rc"
echo "result=${PEAR_SELECTION_RESULT:-}"
INNER
...
PEAR_SELECTION_RESULT=""
unset PEAR_MENU_SORT_MODE PEAR_MENU_SORT_REVERSE PEAR_MENU_META_SIZEKB
set +e
PEAR_MENU_META_EPOCHS="100,200" paginated_multi_select "Test Menu" "Alpha" "Beta" > "$HOME/menu.out" 2> "$HOME/menu.err" < /dev/null
rc=$?
set -e
echo "rc=$rc"
echo "mode=${PEAR_MENU_SORT_MODE:-}"
echo "result=${PEAR_SELECTION_RESULT:-}"
[[ $rc -eq 0 ]]
INNER
...
PEAR_SELECTION_RESULT=""
set +e
PEAR_MENU_META_SIZEKB="1,100" PEAR_MENU_SORT_MODE=size PEAR_MENU_SORT_REVERSE=false paginated_multi_select "Test Menu" "Small" "Large" > "$HOME/menu-default.out" 2> "$HOME/menu-default.err" < /dev/null
default_rc=$?
set -e
echo "default=${PEAR_SELECTION_RESULT:-}"

: > "$key_state"
PEAR_SELECTION_RESULT=""
set +e
NEXT_KEY="CHAR:O" PEAR_MENU_META_SIZEKB="1,100" PEAR_MENU_SORT_MODE=size PEAR_MENU_SORT_REVERSE=false paginated_multi_select "Test Menu" "Small" "Large" > "$HOME/menu-reverse.out" 2> "$HOME/menu-reverse.err" < /dev/null
reverse_rc=$?
set -e
echo "default_rc=$default_rc"
echo "reverse_rc=$reverse_rc"
echo "reverse=${PEAR_SELECTION_RESULT:-}"
[[ $default_rc -eq 0 ]]
[[ $reverse_rc -eq 0 ]]
INNER
...
@test "uninstall main sets PEAR_DELETE_MODE=trash by default" {
	local apps_cache
...
@test "uninstall main sets PEAR_DELETE_MODE=permanent with --permanent flag" {
	local apps_cache
...
_bg_items_runner() {
	HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		...
		PEAR_TEST_MODE=0 PEAR_TEST_NO_AUTH=0 \
