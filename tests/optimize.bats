...
@test "opt_system_maintenance reports DNS and Spotlight" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_DRY_RUN=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
@test "fix_broken_preferences does not count protected Adobe plists" {
	...
	run env HOME="$test_home" PROJECT_ROOT="$PROJECT_ROOT" PE_DEBUG=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/maintenance.sh"
...
@test "opt_fix_broken_configs reports partial results when scan hits its time budget" {
	...
	run env HOME="$test_home" PROJECT_ROOT="$PROJECT_ROOT" PEAR_TIMEOUT_HINT_SCAN_SEC=0 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/maintenance.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
@test "opt_spotlight_index_optimize dry-run reports rebuild when probes are slow" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_DRY_RUN=1 PEAR_OPTIMIZE_SPOTLIGHT_SLOW_SEC=-1 bash --noprofile --norc <<'EOF'
set -euo pipefail
...
<dict>
    ...
    <key>ProgramArguments</key>
    <array>
        <string>/Volumes/PearNonexistentDisk/tool</string>
    </array>
</dict>
</plist>
PLIST
...
periodic() { true; }
export -f periodic
tmplog="$(mktemp /tmp/pear-test-daily.XXXXXX)"
touch "$tmplog"
PEAR_PERIODIC_LOG="$tmplog" opt_periodic_maintenance
rm -f "$tmplog"
EOF
...
periodic() { true; }
export -f periodic
tmpdir="$(mktemp -d /tmp/pear-test-stat-path.XXXXXX)"
mkdir -p "$tmpdir/bin"
...
touch "$tmplog"
PATH="$tmpdir/bin:$PATH" PEAR_PERIODIC_LOG="$tmplog" opt_periodic_maintenance
rm -rf "$tmpdir"
EOF
...
periodic() { true; }
export -f periodic
PEAR_PERIODIC_LOG="/tmp/pear-test-nonexistent-daily.out" opt_periodic_maintenance
EOF
...
@test "run_optimize_diagnostics flags sustained CloudShell as primary bottleneck" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		PEAR_OPTIMIZE_PS_SAMPLE_1=$'120 /Applications/AliEntSafe.app/Contents/Services/CloudShell.app/Contents/MacOS/CloudShell --type=event-capture\n35 /usr/libexec/syspolicyd\n20 /System/Library/PrivateFrameworks/SkyLight.framework/Resources/WindowServer' \
		PEAR_OPTIMIZE_PS_SAMPLE_2=$'140 /Applications/AliEntSafe.app/Contents/Services/CloudShell.app/Contents/MacOS/CloudShell --type=event-processor\n30 /usr/libexec/syspolicyd\n18 /System/Library/PrivateFrameworks/SkyLight.framework/Resources/WindowServer' \
...
@test "run_optimize_diagnostics suppresses one-off CPU spikes" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		PEAR_OPTIMIZE_PS_SAMPLE_1=$'180 /Applications/AliEntSafe.app/Contents/Services/CloudShell.app/Contents/MacOS/CloudShell --type=event-capture' \
		PEAR_OPTIMIZE_PS_SAMPLE_2=$'5 /Applications/AliEntSafe.app/Contents/Services/CloudShell.app/Contents/MacOS/CloudShell --type=event-capture' \
		bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/diagnostics.sh"
...
@test "run_optimize_diagnostics offers user-mounted images under syspolicyd pressure in dry-run" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_DRY_RUN=1 \
		PEAR_OPTIMIZE_PS_SAMPLE_1=$'55 /usr/libexec/syspolicyd' \
		PEAR_OPTIMIZE_PS_SAMPLE_2=$'60 /usr/libexec/syspolicyd' \
		PEAR_OPTIMIZE_SPCTL_STATUS="assessments enabled" \
...
@test "run_optimize_diagnostics keeps healthy runs quiet even with user-mounted images" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_DRY_RUN=1 \
		PEAR_OPTIMIZE_PS_SAMPLE_1=$'1 /usr/sbin/distnoted' \
		PEAR_OPTIMIZE_PS_SAMPLE_2=$'1 /usr/sbin/distnoted' \
		PEAR_OPTIMIZE_HDIUTIL_INFO=$'================================================\nimage-path      : /Users/test/Downloads/TestInstaller.dmg\n/dev/disk14s1\t/Volumes/Test Installer\n' \
		bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
@test "run_optimize_diagnostics honors optimize whitelist paths for mounted images (#977)" {
	...
	cat > "$HOME/.config/pear/whitelist_optimize" <<'EOF'
system_maintenance
...
@test "run_optimize_diagnostics stays quiet when nothing matches" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		PEAR_OPTIMIZE_PS_SAMPLE_1=$'4 /usr/sbin/distnoted\n3 /usr/libexec/coreaudiod' \
		PEAR_OPTIMIZE_PS_SAMPLE_2=$'5 /usr/sbin/distnoted\n2 /usr/libexec/coreaudiod' \
		bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/diagnostics.sh"
...
@test "_login_item_app_exists trusts an existing System Events login item path" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PE_DEBUG=1 HELPER_PATH="$helper" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
mdfind() { return 1; }
...
@test "optimize_sudo_available returns false when sudo session was denied" {
	run env PROJECT_ROOT="$PROJECT_ROOT" PEAR_OPTIMIZE_SUDO_AVAILABLE="false" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
@test "optimize_sudo_available returns false in test mode regardless of optimize entrypoint" {
	# Ad-hoc task invocation under PEAR_TEST_NO_AUTH must hard-deny sudo
	# even when PEAR_OPTIMIZE_SUDO_AVAILABLE was never set by bin/optimize.sh.
	run env PROJECT_ROOT="$PROJECT_ROOT" PEAR_TEST_NO_AUTH=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
unset PEAR_OPTIMIZE_SUDO_AVAILABLE
if optimize_sudo_available; then
...
@test "flush_dns_cache does not invoke sudo under PEAR_TEST_NO_AUTH" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_TEST_NO_AUTH=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
unset PEAR_OPTIMIZE_SUDO_AVAILABLE
...
@test "sudo-required optimize tasks short-circuit without invoking sudo when access denied" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" \
		PEAR_OPTIMIZE_SUDO_AVAILABLE="false" \
		PEAR_DRY_RUN="0" \
		bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
@test "has_active_vpn_interface respects PEAR_ASSUME_VPN_ACTIVE override" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_ASSUME_VPN_ACTIVE=1 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
...
@test "has_active_vpn_interface returns false when PEAR_ASSUME_VPN_ACTIVE=0" {
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_ASSUME_VPN_ACTIVE=0 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/optimize/tasks.sh"
