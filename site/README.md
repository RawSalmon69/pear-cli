...
no build step, no external scripts/CDNs. The one font (Space Grotesk, SIL OFL)
is self-hosted in `assets/fonts/` so nothing loads at runtime from another host.
The Download button resolves the latest `companion-v*` GitHub release
client-side (first `.zip` asset of the newest non-draft, non-prerelease
`companion-v*` tag); with JS off or the API rate-limited it falls back to the
releases page.
