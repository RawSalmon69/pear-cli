...
@test "clean_xcode_documentation_cache keeps newest DeveloperDocumentation index" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_XCODE_DOCUMENTATION_CACHE_DIR="$doc_root" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
note_activity() { :; }
...
@test "clean_xcode_system_coresimulator_caches removes only direct cache children" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_XCODE_SYSTEM_CORESIMULATOR_CACHE_DIR="$cache_root" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
note_activity() { :; }
...
@test "clean_dev_ai_agents respects PEAR_AI_AGENTS_KEEP and skips missing roots" {
	local claude_root="$HOME/.local/share/claude/versions"
	mkdir -p "$claude_root"
...
note_activity() { :; }
safe_clean() { echo "$1"; }
PEAR_AI_AGENTS_KEEP=2 clean_dev_ai_agents
EOF
...
@test "clean_xcode_simulator_runtime_volumes shows scan progress and skips sizing in-use volumes" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_XCODE_SIM_RUNTIME_VOLUMES_ROOT="$volumes_root" PEAR_XCODE_SIM_RUNTIME_CRYPTEX_ROOT="$cryptex_root" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
...
_sim_runtime_mount_points() {
    printf '%s\n' "$PEAR_XCODE_SIM_RUNTIME_VOLUMES_ROOT/in-use-runtime"
}
...
@test "clean_xcode_simulator_runtime_volumes deletes nothing when mount enumeration fails" {
	...
	run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_XCODE_SIM_RUNTIME_VOLUMES_ROOT="$volumes_root" PEAR_XCODE_SIM_RUNTIME_CRYPTEX_ROOT="$HOME/none" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/dev.sh"
