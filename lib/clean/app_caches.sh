    ...
    if [[ "$simulator_running" == "false" ]]; then
        ...
        if command -v xcrun > /dev/null 2>&1; then
            ...
            if declare -F xcrun > /dev/null 2>&1; then
                ...
                unavailable_devices_output=$(run_with_timeout "$PEAR_TIMEOUT_QUICK_DETECT_SEC" xcrun simctl list devices unavailable 2> /dev/null || true)
            ...
            if [[ "$unavail_count" -gt 0 ]]; then
                if [[ "${DRY_RUN:-false}" == "true" ]]; then
                    ...
                    if declare -F xcrun > /dev/null 2>&1; then
                        ...
                        run_with_timeout "$PEAR_TIMEOUT_MEDIUM_PROBE_SEC" xcrun simctl delete unavailable > /dev/null 2>&1 || _delete_rc=$?
                    fi
...
clean_ai_apps() {
    ...
    # was removed outside pe uninstall (#1179). Models live in ~/.lmstudio and
...
# directories whose seg.x0 file is older than PEAR_ORPHAN_AGE_DAYS are removed.
...
clean_neatdm_stale_segments() {
    ...
    for seg_dir in "$neatdm_dir"/*/; do
        ...
        if [[ $age_days -ge ${PEAR_ORPHAN_AGE_DAYS:-30} ]]; then
            stale_dirs+=("$seg_dir")
        fi
    done
