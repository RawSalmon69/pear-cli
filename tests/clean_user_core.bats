...
@test "clean_user_essentials keeps Pear runtime logs while cleaning other user logs" {
    mkdir -p "$HOME/Library/Logs/pear"
    mkdir -p "$HOME/Library/Logs/OtherApp"
    touch "$HOME/Library/Logs/pear/operations.log"
...
[[ -f "$HOME/Library/Logs/pear/operations.log" ]]
...
export PEAR_EXTERNAL_VOLUMES_ROOT="$link_root"
...
@test "clean_app_caches caps precise sandbox size scans when many containers exist" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" DRY_RUN=true PEAR_CONTAINER_CACHE_PRECISE_SIZE_LIMIT=2 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/user.sh"
start_section_spinner() { :; }
