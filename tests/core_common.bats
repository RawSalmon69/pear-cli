...
@test "pe_spinner_chars returns default sequence" {
    result="$(HOME="$HOME" bash --noprofile --norc -c "source '$PROJECT_ROOT/lib/core/common.sh'; pe_spinner_chars")"
    [ "$result" = "|/-\\" ]
}
...
small_kb=1
large_kb=$(((PEAR_ONE_GB_BYTES * 2) / 1024))
...
@test "pear_is_reverse_dns_bundle_id rejects defaults domains and glob-like ids" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"

for valid in "com.example.App" "andriiliakh.Artpaper" "dev.zed.Zed-Nightly" "org.keepassxc.KeePassXC"; do
    pear_is_reverse_dns_bundle_id "$valid" || {
...
for invalid in "-g" "NSGlobalDomain" "com-example" "com.foo.*" "com.foo.[abc]" "unknown" ""; do
    if pear_is_reverse_dns_bundle_id "$invalid"; then
...
@test "pear_name_has_bundle_id_boundary rejects sibling bundle prefixes" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
for valid in \
    ...
    pear_name_starts_with_bundle_id_boundary "$valid" "$bundle_id" || {
...
for invalid in \
    ...
    if pear_name_starts_with_bundle_id_boundary "$invalid" "$bundle_id"; then
...
for valid in \
    ...
    "TEAM.com.example.TestApp.FileProvider" \
    "/tmp/com.example.TestApp.pkg.bom"; do
    pear_name_has_bundle_id_boundary "$valid" "$bundle_id" || {
...
for invalid in \
    ...
    if pear_name_has_bundle_id_boundary "$invalid" "$bundle_id"; then
...
@test "log_info prints message and appends to log file" {
    ...
    local log_file="$HOME/Library/Logs/pear/pear.log"
    [[ -f "$log_file" ]]
...
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
rm -rf "$HOME/Library/Logs/pear"
log_operation "clean" "REMOVED" "/tmp/example" "1KB"
EOF
    [ "$status" -eq 0 ]

    local oplog="$HOME/Library/Logs/pear/operations.log"
    [[ -f "$oplog" ]]
    grep -Fq "[clean] REMOVED /tmp/example (1KB)" "$oplog"
}

@test "should_protect_path protects Pear runtime logs" {
    result="$(
        HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" bash --noprofile --norc -c \
            'source "$PROJECT_ROOT/lib/core/common.sh"; should_protect_path "$HOME/Library/Logs/pear/operations.log" && echo protected || echo not-protected'
    )"
    [ "$result" = "protected" ]
}
...
@test "rotate_log_once only checks log size once per session" {
    ...
    result=$(HOME="$HOME" PEAR_LOG_ROTATED=1 bash --noprofile --norc -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$PEAR_LOG_ROTATED")
    [[ "$result" == "1" ]]
}
...
@test "Karabiner-Elements is protected during cleanup but allowed for uninstall" {
    ...
    # The main app bundle id (the actual `pe uninstall --list` key) behaves the same:
...
source "$PROJECT_ROOT/lib/core/common.sh"
PEAR_SPINNER_PREFIX="  " start_inline_spinner "Testing..."
sleep 0.1
stop_inline_spinner
echo "done"
EOF
...
@test "read_key maps j/k/h/l to navigation" {
    ...
    run bash -c "export PEAR_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'k' | read_key"
    [ "$output" = "UP" ]

    run bash -c "export PEAR_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'h' | read_key"
    [ "$output" = "LEFT" ]

    run bash -c "export PEAR_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'l' | read_key"
    [ "$output" = "RIGHT" ]
}
...
@test "read_key respects PEAR_READ_KEY_FORCE_CHAR" {
    run bash -c "export PEAR_BASE_LOADED=1; export PEAR_READ_KEY_FORCE_CHAR=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'j' | read_key"
    [ "$output" = "CHAR:j" ]
}

@test "read_key keeps Ctrl-C as quit when forcing printable characters" {
    run bash -c "export PEAR_BASE_LOADED=1; export PEAR_READ_KEY_FORCE_CHAR=1; source '$PROJECT_ROOT/lib/core/ui.sh'; printf '\\003' | read_key"
    [ "$output" = "QUIT" ]
}

@test "ensure_sudo_session returns 1 and sets PEAR_SUDO_ESTABLISHED=false in test mode" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_TEST_NO_AUTH=1 bash --noprofile --norc <<'SCRIPT'
source "$PROJECT_ROOT/lib/core/base.sh"
source "$PROJECT_ROOT/lib/core/sudo.sh"
PEAR_SUDO_ESTABLISHED=""
ensure_sudo_session "Test prompt" && rc=0 || rc=$?
echo "EXIT=$rc"
echo "FLAG=$PEAR_SUDO_ESTABLISHED"
SCRIPT
...
@test "adopt_sudo_session starts keepalive for cached sudo" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_TEST_MODE=0 PEAR_TEST_NO_AUTH=0 bash --noprofile --norc <<'SCRIPT'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/base.sh"
source "$PROJECT_ROOT/lib/core/sudo.sh"
...
echo "EXIT=$?"
echo "FLAG=$PEAR_SUDO_ESTABLISHED"
echo "PID=$PEAR_SUDO_KEEPALIVE_PID"
SCRIPT
