...
@test "prepare_pear_tmpdir uses writable TMPDIR when available" {
    local writable_tmp="$HOME/custom-tmp"
    mkdir -p "$writable_tmp"

    result=$(env HOME="$HOME" TMPDIR="$writable_tmp" bash -c "source '$PROJECT_ROOT/lib/core/base.sh'; prepare_pear_tmpdir")
    [ "$result" = "$writable_tmp" ]
}

@test "prepare_pear_tmpdir falls back to user cache when TMPDIR is not writable" {
    local blocked_tmp="$HOME/blocked-tmp"
    mkdir -p "$blocked_tmp"
    chmod 500 "$blocked_tmp"

    result=$(env HOME="$HOME" TMPDIR="$blocked_tmp" bash -c "source '$PROJECT_ROOT/lib/core/base.sh'; prepare_pear_tmpdir")
    [ "$result" = "$HOME/.cache/pear/tmp" ]
    [ -d "$HOME/.cache/pear/tmp" ]
}

@test "ensure_pear_temp_root caches the first resolved directory" {
    ...
    result=$(env HOME="$HOME" TMPDIR="$first_tmp" bash -c "
        ...
        first=\$PEAR_RESOLVED_TMPDIR
        export TMPDIR='$second_tmp'
        ensure_pear_temp_root
        second=\$PEAR_RESOLVED_TMPDIR
        printf '%s|%s\n' \"\$first\" \"\$second\"
    ")
...
PEAR_RESOLVED_TMPDIR="$HOME/.cache/pear/tmp"
PEAR_TEMP_REGISTRY_FILE=""
export PEAR_RESOLVED_TMPDIR PEAR_TEMP_REGISTRY_FILE
mkdir -p "$PEAR_RESOLVED_TMPDIR"
...
persistent_file="$HOME/.cache/pear/installed_apps_cache"
touch "$persistent_file"
printf '%s\n' "$persistent_file" > "$PEAR_TEMP_REGISTRY_FILE"

cleanup_temp_files
[[ -e "$persistent_file" ]]

invalid_registry="$HOME/.cache/pear/not-a-temp-registry"
printf '%s\n' "$persistent_file" > "$invalid_registry"
PEAR_TEMP_REGISTRY_FILE="$invalid_registry"
cleanup_temp_files
[[ -e "$invalid_registry" ]]
EOF
...
@test "prune_stale_pear_temp_files leaves persistent cache and fresh temps alone (#1203)" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_TEMP_STALE_MINUTES=60 bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/base.sh"

temp_root="$HOME/.cache/pear/tmp"
old_temp="$temp_root/old-scan"
fresh_temp="$temp_root/current-scan"
old_spinner="$temp_root/.pear-spinner.old"
fresh_spinner="$temp_root/.pear-spinner.fresh"
persistent_cache="$HOME/.cache/pear/installed_apps_cache"
...
prune_stale_pear_temp_files "$temp_root"
...
@test "prepare_pear_tmpdir falls back to /tmp when TMPDIR and invoking home are unavailable" {
...
@test "ensure_user_dir creates nested directory" {
    test_dir="$HOME/.config/pear/deep/nested/path"
    bash -c "source '$PROJECT_ROOT/lib/core/base.sh'; ensure_user_dir '$test_dir'"
    [ -d "$test_dir" ]
}
...
@test "ensure_user_file creates file and parent directories" {
    test_file="$HOME/.config/pear/test.log"
...
@test "multiple ensure_user_file calls in same directory" {
    bash -c "source '$PROJECT_ROOT/lib/core/base.sh'
        ensure_user_file '$HOME/.config/pear/file1.txt'
        ensure_user_file '$HOME/.config/pear/file2.txt'
        ensure_user_file '$HOME/.config/pear/file3.txt'
    "

    [ -f "$HOME/.config/pear/file1.txt" ]
    [ -f "$HOME/.config/pear/file2.txt" ]
    [ -f "$HOME/.config/pear/file3.txt" ]
}
