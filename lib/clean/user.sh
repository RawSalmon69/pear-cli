...
clean_trash() {
    ...
    # blocked pe clean on user confirmation. Volume Trashes
    # (/Volumes/*/.Trashes/<uid>/) are not handled here; pe clean only manages
    ...
    if [[ -t 1 ]]; then
        PEAR_SPINNER_PREFIX="  " start_inline_spinner "Emptying trash, ${trash_count} items..."
    fi
...
clean_user_essentials() {
    ...
    if [[ "${PEAR_SKIP_TRASH_CLEANUP:-0}" != "1" ]]; then
        clean_trash
    fi
    stop_section_spinner
...
_clean_mail_downloads() {
    local mail_age_days=${PEAR_MAIL_AGE_DAYS:-}
    ...
    [[ "${DRY_RUN:-false}" == "true" || "${PEAR_DRY_RUN:-0}" == "1" ]] && dry_run_mode=true
    for target_path in "${mail_dirs[@]}"; do
        if [[ -d "$target_path" ]]; then
            ...
            local min_kb="${PEAR_MAIL_DOWNLOADS_MIN_KB:-}"
            if ! [[ "$min_kb" =~ ^[0-9]+$ ]]; then
                min_kb=5120
            fi
            ...
            while IFS= read -r -d '' file_path; do
                if [[ -f "$file_path" ]]; then
                    ...
                    if [[ "$dry_run_mode" == "true" ]]; then
                        PEAR_DRY_RUN=1 safe_remove "$file_path" true "$file_size_kb" && remove_rc=0
...
_clean_darwin_user_runtime_dir() {
    ...
    local kind="$2"
    local label="$3"
    local age_days="${PEAR_DARWIN_USER_RUNTIME_AGE_DAYS:-7}"
    local max_items="${PEAR_DARWIN_USER_RUNTIME_MAX_ITEMS:-1500}"
    local scan_timeout="${PEAR_DARWIN_USER_RUNTIME_SCAN_TIMEOUT:-8}"
...
_clean_darwin_user_runtime_dirs() {
    if [[ "${PEAR_TEST_MODE:-0}" == "1" || "${PEAR_TEST_NO_AUTH:-0}" == "1" ]]; then
        [[ "${PEAR_ENABLE_DARWIN_RUNTIME_CLEANUP_IN_TESTS:-0}" == "1" ]] || return 0
    fi
...
clean_chrome_old_versions() {
    local -a app_paths
    if [[ -n "${PEAR_CHROME_APP_PATHS:-}" ]]; then
        IFS=':' read -ra app_paths <<< "$PEAR_CHROME_APP_PATHS"
    else
        app_paths=(
...
clean_edge_old_versions() {
    ...
    local -a app_paths
    if [[ -n "${PEAR_EDGE_APP_PATHS:-}" ]]; then
        IFS=':' read -ra app_paths <<< "$PEAR_EDGE_APP_PATHS"
    else
        app_paths=(
...
clean_brave_old_versions() {
    local -a app_paths
    if [[ -n "${PEAR_BRAVE_APP_PATHS:-}" ]]; then
        IFS=':' read -ra app_paths <<< "$PEAR_BRAVE_APP_PATHS"
    else
        app_paths=(
...
clean_support_app_data() {
    local support_age_days="${PEAR_SUPPORT_CACHE_AGE_DAYS:-30}"
...
clean_app_caches() {
    ...
    local cleaned_count=0
    local found_any=false
    local precise_size_limit="${PEAR_CONTAINER_CACHE_PRECISE_SIZE_LIMIT:-64}"
...
external_volume_root() {
    printf '%s\n' "${PEAR_EXTERNAL_VOLUMES_ROOT:-/Volumes}"
}
...
validate_external_volume_target() {
    ...
    local disk_info=""
    disk_info=$(run_with_timeout "$PEAR_TIMEOUT_QUICK_DETECT_SEC" command diskutil info "$resolved" 2> /dev/null || echo "")
    if [[ -n "$disk_info" ]]; then
...
clean_external_volume_target() {
    ...
    local metadata_scan_timeout="${PEAR_EXTERNAL_VOLUME_SCAN_TIMEOUT:-15}"
...
clean_tart_caches() {
    ...
    if [[ "${DRY_RUN:-false}" == "true" ]]; then
        echo -e "  ${YELLOW}${ICON_DRY_RUN}${NC} Tart caches · would prune items older than ${PEAR_ORPHAN_AGE_DAYS} days (${cache_size_human})"
        echo -e "    ${GRAY}tart prune --entries caches --older-than ${PEAR_ORPHAN_AGE_DAYS}${NC}"
        note_activity
        return 0
    fi
    ...
    if run_with_timeout "$PEAR_TIMEOUT_PKG_CLEANUP_SEC" tart prune --entries caches --older-than "$PEAR_ORPHAN_AGE_DAYS" > /dev/null 2>&1; then
    ...
    if [[ "$prune_succeeded" != "true" ]]; then
        ...
        debug_log "tart prune failed for cache-only ${PEAR_ORPHAN_AGE_DAYS}-day policy"
        note_activity
        return 0
    fi
...
clean_application_support_logs() {
    ...
    local cleaned_count=0
    local found_any=false
    local size_timeout_seconds="${PEAR_APP_SUPPORT_ITEM_SIZE_TIMEOUT_SEC:-0.4}"
...
report_agent_worktree_candidates() {
    ...
    for root in "${roots[@]}"; do
        ...
        while IFS= read -r -d '' container; do
            ...
            note_activity
        done < <(run_with_timeout "$PEAR_TIMEOUT_PKG_CLEANUP_SEC" command find "$root" -maxdepth 6 -type d -path "*/.claude/worktrees" -prune -print0 2> /dev/null)
    done
    return 0
}
...
check_large_file_candidates() {
    ...
    _large_candidate_size_kb() {
        local path="$1"
        local timeout_seconds="${2:-${PEAR_LARGE_CANDIDATE_SIZE_TIMEOUT:-3}}"
    ...
    if [[ "${SYSTEM_CLEAN:-false}" != "true" ]] && command -v tmutil > /dev/null 2>&1 &&
        ...
        snapshot_list=$(run_with_timeout "$PEAR_TIMEOUT_SHORT_QUERY_SEC" tmutil listlocalsnapshots / 2> /dev/null || true)
    ...
    if command -v docker > /dev/null 2>&1; then
        local docker_output
        docker_output=$(run_with_timeout "$PEAR_TIMEOUT_SHORT_QUERY_SEC" docker system df --format '{{.Type}}\t{{.Size}}\t{{.Reclaimable}}' 2> /dev/null || true)
        if [[ -n "$docker_output" ]]; then
            ...
            docker_output=$(run_with_timeout "$PEAR_TIMEOUT_SHORT_QUERY_SEC" docker system df 2> /dev/null || true)
            if [[ -n "$docker_output" ]]; then
                stop_section_spinner
    ...
    _report_large_review_dir "iOS backups" "$HOME/Library/Application Support/MobileSync/Backup" "$PEAR_TIMEOUT_HINT_SCAN_SEC"
