...
setup() {
    ...
    rm -rf "$HOME/.config"
    mkdir -p "$HOME"
    WHITELIST_PATH="$HOME/.config/pear/whitelist"
}
...
@test "load_whitelist falls back to defaults when config missing" {
    rm -f "$WHITELIST_PATH"
    HOME="$HOME" bash --noprofile --norc -c "source '$PROJECT_ROOT/lib/manage/whitelist.sh'; rm -f \"\$HOME/.config/pear/whitelist\"; load_whitelist; printf '%s\n' \"\${CURRENT_WHITELIST_PATTERNS[@]}\"" > "$HOME/current_whitelist.txt"
...
@test "pe clean --whitelist persists selections" {
    whitelist_file="$HOME/.config/pear/whitelist"
    mkdir -p "$(dirname "$whitelist_file")"

    run bash --noprofile --norc -c "cd '$PROJECT_ROOT'; printf \$'\\n' | HOME='$HOME' ./pe clean --whitelist"
    [ "$status" -eq 0 ]
...
@test "pe clean --whitelist cancel preserves existing file (#807)" {
    whitelist_file="$HOME/.config/pear/whitelist"
    mkdir -p "$(dirname "$whitelist_file")"
    ...
    run bash --noprofile --norc -c "cd '$PROJECT_ROOT'; printf 'q' | HOME='$HOME' ./pe clean --whitelist"
    [ "$status" -eq 0 ]
    [[ "$output" == *"Cancelled"* ]]
...
@test "default whitelist protects tealdeer cache parent for tldr pages" {
    ...
    if HOME="$HOME" bash --noprofile --norc -c "
        source '$PROJECT_ROOT/lib/manage/whitelist.sh'
        rm -f \"\$HOME/.config/pear/whitelist\"
        load_whitelist
