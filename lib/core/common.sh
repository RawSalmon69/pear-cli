#!/bin/bash
# Pear - Common Functions Library
# Main entry point that loads all core modules

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${PEAR_COMMON_LOADED:-}" ]]; then
    return 0
fi
readonly PEAR_COMMON_LOADED=1

_PEAR_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Load core modules
source "$_PEAR_CORE_DIR/base.sh"
prepare_pear_tmpdir > /dev/null
source "$_PEAR_CORE_DIR/log.sh"

source "$_PEAR_CORE_DIR/timeout.sh"
source "$_PEAR_CORE_DIR/timeouts.sh"
source "$_PEAR_CORE_DIR/file_ops.sh"
source "$_PEAR_CORE_DIR/help.sh"
source "$_PEAR_CORE_DIR/ui.sh"
source "$_PEAR_CORE_DIR/app_protection.sh"
source "$_PEAR_CORE_DIR/bundle_resolver.sh"
source "$_PEAR_CORE_DIR/pkg_receipts.sh"

# Load sudo management if available
if [[ -f "$_PEAR_CORE_DIR/sudo.sh" ]]; then
    source "$_PEAR_CORE_DIR/sudo.sh"
fi
...
pear_path_identity() {
    local path="$1"
    local normalized
    normalized=$(pear_normalize_path "$path")
...
update_via_homebrew() {
    ...
    local brew_update_timeout="${PEAR_HOMEBREW_UPDATE_TIMEOUT:-120}"
    ...
    # Upgrade Pear
    if [[ -t 1 ]]; then
        start_inline_spinner "Upgrading Pear..."
    else
        echo "Upgrading Pear..."
    fi

    local brew_upgrade_timeout="${PEAR_HOMEBREW_UPGRADE_TIMEOUT:-120}"
    HOMEBREW_NO_ENV_HINTS=1 HOMEBREW_NO_AUTO_UPDATE=1 NONINTERACTIVE=1 \
        run_with_timeout "$brew_upgrade_timeout" brew upgrade pear > "$temp_upgrade" 2>&1 || true
    ...
    if echo "$upgrade_output" | grep -q "already installed"; then
        ...
        installed_version=$(HOMEBREW_NO_ENV_HINTS=1 HOMEBREW_NO_AUTO_UPDATE=1 \
            run_with_timeout "$PEAR_TIMEOUT_PKG_LIST_SEC" brew list --versions pear 2> /dev/null | awk '{print $2}')
        [[ -z "$installed_version" ]] && installed_version=$(pe --version 2> /dev/null | awk '/Pear version/ {print $3; exit}')
        echo ""
        ...
        new_version=$(HOMEBREW_NO_ENV_HINTS=1 HOMEBREW_NO_AUTO_UPDATE=1 \
            run_with_timeout "$PEAR_TIMEOUT_PKG_LIST_SEC" brew list --versions pear 2> /dev/null | awk '{print $2}')
        [[ -z "$new_version" ]] && new_version=$(pe --version 2> /dev/null | awk '/Pear version/ {print $3; exit}')
        echo ""
    ...
    rm -f "$HOME/.cache/pear/version_check" "$HOME/.cache/pear/update_message" 2> /dev/null || true
}
