#!/bin/bash
# Pear - Base Definitions and Utilities
# Core definitions, constants, and basic utility functions used by all modules

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${PEAR_BASE_LOADED:-}" ]]; then
    return 0
fi
readonly PEAR_BASE_LOADED=1
...
# ============================================================================
readonly PEAR_TEMP_FILE_AGE_DAYS=7       # Temp file retention (days)
readonly PEAR_ORPHAN_AGE_DAYS=30         # Orphaned data retention (days)
readonly PEAR_DOTDIR_ORPHAN_AGE_DAYS=60  # Orphan dotfile hint threshold (days)
readonly PEAR_MAX_PARALLEL_JOBS=15       # Parallel job limit
readonly PEAR_MAIL_DOWNLOADS_MIN_KB=5120 # Mail attachment size threshold
readonly PEAR_MAIL_AGE_DAYS=30           # Mail attachment retention (days)
readonly PEAR_LOG_AGE_DAYS=7             # Log retention (days)
readonly PEAR_CRASH_REPORT_AGE_DAYS=7    # Crash report retention (days)
readonly PEAR_SAVED_STATE_AGE_DAYS=30    # Saved state retention (days) - increased for safety
readonly PEAR_GPU_CACHE_AGE_DAYS=1       # Rebuildable GPU cache retention (days)
readonly PEAR_TM_BACKUP_SAFE_HOURS=48    # TM backup safety window (hours)
readonly PEAR_MAX_DS_STORE_FILES=500     # Max .DS_Store files to clean per scan
readonly PEAR_MAX_ORPHAN_ITERATIONS=100  # Max iterations for orphaned app data scan
readonly PEAR_ONE_GIB_KB=$((1024 * 1024))
readonly PEAR_ONE_GB_BYTES=1000000000
...
detect_architecture() {
    if [[ -n "${PEAR_ARCH_CACHE:-}" ]]; then
        echo "$PEAR_ARCH_CACHE"
        return 0
    fi

    if [[ "$(uname -m)" == "arm64" ]]; then
        export PEAR_ARCH_CACHE="Apple Silicon"
    else
        export PEAR_ARCH_CACHE="Intel"
    fi
    echo "$PEAR_ARCH_CACHE"
}
...
get_optimal_parallel_jobs() {
    ...
    if [[ -z "${PEAR_CPU_CORES_CACHE:-}" ]]; then
        export PEAR_CPU_CORES_CACHE=$(sysctl -n hw.ncpu 2> /dev/null || echo 4)
    fi
    local cpu_cores="$PEAR_CPU_CORES_CACHE"
...
pear_name_starts_with_bundle_id_boundary() {
    local name="${1##*/}"
    local bundle_id="${2:-}"
...
pear_name_has_bundle_id_boundary() {
    local name="${1##*/}"
    local bundle_id="${2:-}"

    pear_name_starts_with_bundle_id_boundary "$name" "$bundle_id" && return 0
    pear_is_reverse_dns_bundle_id "$bundle_id" || return 1
    [[ "$name" == *."$bundle_id" ||
        "$name" == *."$bundle_id".* ]]
}
...
declare -a PEAR_TEMP_FILES=()
declare -a PEAR_TEMP_DIRS=()
...
probe_temp_root() {
    ...
    probe=$(mktemp "$path/pear.probe.XXXXXX" 2> /dev/null) || return 1
    rm -f "$probe" 2> /dev/null || true

    printf '%s\n' "$path"
}

# Remove abandoned files only from Pear's dedicated fallback temp directory.
...
# Pear processes while bounding leftovers from interrupted runs.
prune_stale_pear_temp_files() {
    local root="${1:-}"
    local invoking_home=""
    local max_age_minutes="${PEAR_TEMP_STALE_MINUTES:-1440}"
...
initialize_pear_temp_registry_path() {
    [[ -n "${PEAR_TEMP_REGISTRY_FILE:-}" ]] && return 0
    [[ -n "${PEAR_RESOLVED_TMPDIR:-}" ]] || return 1
    ...
    PEAR_TEMP_REGISTRY_FILE="${PEAR_RESOLVED_TMPDIR%/}/pear.registry.$$"
    export PEAR_TEMP_REGISTRY_FILE
}

ensure_pear_temp_registry_file() {
    initialize_pear_temp_registry_path || return 1

    case "$PEAR_TEMP_REGISTRY_FILE" in
        "${PEAR_RESOLVED_TMPDIR%/}"/pear.registry.*) ;;
        *) return 1 ;;
    esac

    if [[ ! -e "$PEAR_TEMP_REGISTRY_FILE" ]]; then
        (umask 077 && set -C && : > "$PEAR_TEMP_REGISTRY_FILE") 2> /dev/null || true
    fi

    [[ -f "$PEAR_TEMP_REGISTRY_FILE" && ! -L "$PEAR_TEMP_REGISTRY_FILE" && -O "$PEAR_TEMP_REGISTRY_FILE" ]]
}
...
ensure_pear_temp_root() {
    ...
    if [[ -z "$resolved" ]]; then
        ...
        if [[ -n "$invoking_home" ]]; then
            resolved=$(probe_temp_root "$invoking_home/.cache/pear/tmp" true || true)
        fi
    fi
...
prepare_pear_tmpdir() {
    ensure_pear_temp_root
    export TMPDIR="$PEAR_RESOLVED_TMPDIR"
    printf '%s\n' "$PEAR_RESOLVED_TMPDIR"
}

pear_temp_path_template() {
    local prefix="${1:-pear}"
    ensure_pear_temp_root
    printf '%s/%s.XXXXXX\n' "$PEAR_RESOLVED_TMPDIR" "$prefix"
}
...
create_temp_file() {
    local temp
    ensure_pear_temp_root
    temp=$(mktemp "$PEAR_RESOLVED_TMPDIR/pear.XXXXXX") || return 1
...
create_temp_dir() {
    local temp
    ensure_pear_temp_root
    temp=$(mktemp -d "$PEAR_RESOLVED_TMPDIR/pear.XXXXXX") || return 1
...
register_temp_file() {
    PEAR_TEMP_FILES+=("$1")
    if ensure_pear_temp_registry_file; then
        printf '%s\n' "$1" >> "$PEAR_TEMP_REGISTRY_FILE" 2> /dev/null || true
    fi
}
...
mktemp_file() {
    ...
    if ! error_msg=$(mktemp "$(pear_temp_path_template "$prefix")" 2>&1); then
...
cleanup_temp_files() {
    ...
    local file
    if [[ ${#PEAR_TEMP_FILES[@]} -gt 0 ]]; then
        for file in "${PEAR_TEMP_FILES[@]}"; do
            [[ -f "$file" ]] && rm -f "$file" 2> /dev/null || true
        done
    fi

    if [[ ${#PEAR_TEMP_DIRS[@]} -gt 0 ]]; then
        for file in "${PEAR_TEMP_DIRS[@]}"; do
    ...
    if ensure_pear_temp_registry_file; then
        ...
        while IFS= read -r registered_path; do
            ...
            [[ "$registered_path" == "${PEAR_RESOLVED_TMPDIR%/}/"* ]] || continue
            [[ ! "$registered_path" =~ (^|/)\.\.(\/|$) ]] || continue

            if [[ -d "$registered_path" && ! -L "$registered_path" ]]; then
                rm -rf "$registered_path" 2> /dev/null || true # SAFE: mktemp dir registered under resolved Pear temp root
            else
                rm -f "$registered_path" 2> /dev/null || true
            fi
        done < "$PEAR_TEMP_REGISTRY_FILE"
        rm -f "$PEAR_TEMP_REGISTRY_FILE" 2> /dev/null || true
    fi
...
#                        when piped or under PE_DEBUG), appends '=== title ==='
...
is_ansi_supported() {
    if [[ -n "${PEAR_ANSI_SUPPORTED_CACHE:-}" ]]; then
        return "$PEAR_ANSI_SUPPORTED_CACHE"
    fi

    # Check if running in interactive terminal
    if ! [[ -t 1 ]]; then
        export PEAR_ANSI_SUPPORTED_CACHE=1
        return 1
    fi
    ...
    case "$TERM" in
        xterm* | vt100 | vt220 | screen* | tmux* | ansi | linux | rxvt* | konsole*)
            export PEAR_ANSI_SUPPORTED_CACHE=0
            return 0
            ;;
        dumb | unknown)
            export PEAR_ANSI_SUPPORTED_CACHE=1
