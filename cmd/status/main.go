...
func getConfigPath() string {
	...
	return filepath.Join(home, ".config", "pear", "status_prefs")
}
...
func (m model) View() string {
	...
	header, pear := renderHeader(m.metrics, m.errMessage, m.animFrame, termWidth, m.catHidden)
	...
	// Combine header, pear, and cards with consistent spacing
	parts := []string{header}
	if alertBar != "" {
		parts = append(parts, alertBar)
	}
	if pear != "" {
		parts = append(parts, pear)
	}
