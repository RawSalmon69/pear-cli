...
@test "scan_installers_in_path (fallback find): honors PEAR_INSTALLER_SCAN_MAX_DEPTH" {
	mkdir -p "$HOME/Downloads/level1"
	touch "$HOME/Downloads/top.dmg"
	touch "$HOME/Downloads/level1/nested.dmg"

	run env PATH="/usr/bin:/bin" PEAR_INSTALLER_SCAN_MAX_DEPTH=1 bash -euo pipefail -c "
        export PEAR_TEST_MODE=1
        source \"\$1\"
        scan_installers_in_path \"\$2\"
    " bash "$PROJECT_ROOT/bin/installer.sh" "$HOME/Downloads"
...
@test "delete_selected_installers removes selected files and records successes" {
	...
	run env HOME="$HOME" TERM="$TERM" bash -euo pipefail -c '
        ...
        export PEAR_DELETE_LOG="$HOME/deletions.log"
        source "$1"

        INSTALLER_PATHS=("$2" "$3")
        INSTALLER_SIZES=(3 3)
        PEAR_SELECTION_RESULT="0,1"
        ...
        [[ ! -e "$2" ]]
        [[ ! -e "$3" ]]
        grep -F "[installer] REMOVED $2" "$HOME/Library/Logs/pear/operations.log" > /dev/null
