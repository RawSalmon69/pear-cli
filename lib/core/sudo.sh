...
request_sudo_access() {
    ...
    if [[ "$is_gui_mode" == true ]]; then
        ...
        local password
        password=$(osascript -e "display dialog \"$escaped_msg\" default answer \"\" with title \"Pear\" with icon caution with hidden answer" -e 'text returned of result' 2> /dev/null)
...
adopt_sudo_session() {
    ...
    if [[ "$PEAR_SUDO_ESTABLISHED" == "true" && -n "$PEAR_SUDO_KEEPALIVE_PID" ]]; then
        if has_sudo_session; then
            return 0
        fi
        _stop_sudo_keepalive "$PEAR_SUDO_KEEPALIVE_PID"
        PEAR_SUDO_KEEPALIVE_PID=""
        PEAR_SUDO_ESTABLISHED="false"
    fi
    ...
    if [[ -n "$PEAR_SUDO_KEEPALIVE_PID" ]]; then
        _stop_sudo_keepalive "$PEAR_SUDO_KEEPALIVE_PID"
        PEAR_SUDO_KEEPALIVE_PID=""
    fi

    PEAR_SUDO_KEEPALIVE_PID=$(_start_sudo_keepalive)
...
ensure_sudo_session() {
    ...
    if has_sudo_session && [[ "$PEAR_SUDO_ESTABLISHED" == "true" ]]; then
        return 0
    fi
