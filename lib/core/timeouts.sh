#!/bin/bash
# Pear - Centralized timeout constants for run_with_timeout calls.
#
...
#
# Naming: PEAR_TIMEOUT_<CATEGORY>_SEC. All values are seconds (integer or
...
if [[ -n "${PEAR_TIMEOUTS_LOADED:-}" ]]; then
    return 0
fi
readonly PEAR_TIMEOUTS_LOADED=1

readonly PEAR_TIMEOUT_QUICK_DETECT_SEC="${PEAR_TIMEOUT_QUICK_DETECT_SEC:-2}"
readonly PEAR_TIMEOUT_SHORT_QUERY_SEC="${PEAR_TIMEOUT_SHORT_QUERY_SEC:-3}"
readonly PEAR_TIMEOUT_MEDIUM_PROBE_SEC="${PEAR_TIMEOUT_MEDIUM_PROBE_SEC:-5}"
readonly PEAR_TIMEOUT_PKG_LIST_SEC="${PEAR_TIMEOUT_PKG_LIST_SEC:-10}"
readonly PEAR_TIMEOUT_PKG_CLEANUP_SEC="${PEAR_TIMEOUT_PKG_CLEANUP_SEC:-20}"
readonly PEAR_TIMEOUT_DISK_VERIFY_SEC="${PEAR_TIMEOUT_DISK_VERIFY_SEC:-30}"
readonly PEAR_TIMEOUT_HINT_SCAN_SEC="${PEAR_TIMEOUT_HINT_SCAN_SEC:-15}"
