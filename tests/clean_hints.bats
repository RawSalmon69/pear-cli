...
@test "probe_project_artifact_hints reuses purge targets and excludes noisy names" {
    ...
    printf '%s\n' "$root" > "$HOME/.config/pear/purge_paths"
...
@test "probe_project_artifact_hints stops at the wall-clock budget (#1053)" {
    ...
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" PEAR_TIMEOUT_HINT_SCAN_SEC=0 \
...
brew() { return 0; }
export -f brew
_PEAR_DOTDIR_OWNER_APP_ROOTS=("$FAKE_APPS_ROOT")
show_orphan_dotdir_hint_notice
EOTD
...
export -f brew
_PEAR_DOTDIR_OWNER_APP_ROOTS=("$EMPTY_APPS_ROOT")
show_orphan_dotdir_hint_notice
EOTD
